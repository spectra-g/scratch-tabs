...
export function convertToXml(json: any, rootName: string = 'root', indent: number = 0): string {
  ...
  function processValue(value: any, name: string): string {
    ...
    const childElements = Object.entries(value).map(([key, val]) => {
  ...
  const rootAttributes = 'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"';
