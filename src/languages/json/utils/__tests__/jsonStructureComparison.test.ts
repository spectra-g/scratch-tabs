import { compareStructures, ComparisonOptions } from '../jsonStructureComparison';
...
describe('JSON Structure Comparison', () => {
  ...
  describe('Array Comparisons', () => {
    ...
    test('should detect array length differences when strict mode is enabled', () => {
      ...
      const result = compareStructures(source, target, { strictArrayLength: true });
      expect(result.matches).toBe(false);
      expect(result.diffList).toHaveLength(1);
    ...
    test('should not detect array length differences when strict mode is disabled', () => {
      const source = '[{"name": "John"}, {"name": "Jane"}]';
      const target = '[{"name": "Bob"}]';
      const result = compareStructures(source, target, { strictArrayLength: false });
      expect(result.matches).toBe(false); // Still false because of missing 'name' key in second element
      expect(result.diffList.some(diff => diff.type === 'ARRAY_LENGTH_MISMATCH')).toBe(false);
    });

    test('should detect polymorphic arrays', () => {
      const source = '[{"type": "user", "name": "John"}, {"type": "admin", "permissions": ["read", "write"]}]';
      const target = '[{"type": "user", "name": "Jane"}, {"type": "admin", "permissions": ["read"]}]';
      const result = compareStructures(source, target);
      expect(result.matches).toBe(false);
      expect(result.diffList.some(diff => diff.type === 'POLYMORPHIC_ARRAY')).toBe(true);
    });

    test('should handle array element differences', () => {
      const source = '[{"name": "John", "age": 30}, {"name": "Jane", "age": 25}]';
      const target = '[{"name": "Bob"}, {"name": "Alice"}]';
      const result = compareStructures(source, target);
      expect(result.matches).toBe(false);
      expect(result.diffList).toHaveLength(2); // Two missing 'age' keys
      expect(result.diffList.every(diff => diff.type === 'MISSING_KEY_RIGHT')).toBe(true);
    });
  });
  ...
  describe('Case Sensitivity', () => {
    ...
    test('should be case insensitive when configured', () => {
      const source = '{"Name": "John", "Age": 30}';
      const target = '{"name": "John", "age": 30}';
      const result = compareStructures(source, target, { caseSensitiveKeys: false });
      expect(result.matches).toBe(true);
    });
  });
  ...
  describe('Array Sample Count', () => {
    test('should respect array sample count setting', () => {
      const source = '[{"name": "John"}, {"name": "Jane"}, {"name": "Bob"}, {"name": "Alice"}]';
      const target = '[{"name": "John"}, {"name": "Jane"}, {"name": "Bob"}, {"name": "Alice", "age": 30}]';
  ...
  describe('Diff Tree Structure', () => {
    test('should generate correct diff tree', () => {
      ...
      const ageNode = userNode.children!.find(child => child.path === '/user/age');
