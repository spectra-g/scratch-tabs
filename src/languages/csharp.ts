 ...
 * C# language detector
 */
export class CsharpLanguageDetector extends BaseLanguageDetector implements LanguageDetector {
  id = 'csharp';
  name = 'C#';
...
`;
  ...
  detect(content: string): DetectionResult {
    ...
    const definitivePatterns = [
      ...
      { pattern: /\b(List|Dictionary|IEnumerable)<[^>]+>/g, weight: 0.2, perMatch: 0.05 }, // Common generic collections
      ...
      { pattern: /\b(var|dynamic|nameof|yield|get|set|value|add|remove|async|await|delegate|event|interface|enum|struct|record|sealed|virtual|override|abstract|partial|params|ref|out|in|is|as|typeof|sizeof|checked|unchecked|unsafe|stackalloc|lock|fixed|volatile|implicit|explicit|operator)\b/g, weight: 0.02, perMatch: 0.01, maxMatches: 20 }, // C# specific keywords (low individual weight, but many add up)
    ...
    if (content.includes("using System;") && content.includes("namespace") && content.includes("class") && content.includes("static void Main")) {
