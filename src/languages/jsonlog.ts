 ...
 */
export class JsonLogLanguageDetector
  ...
  implements LanguageDetector
{
  ...
  detect(content: string): DetectionResult {
      ...
      try {
        ...
        if (
          ...
          const hasServiceId = keys.some((key) =>
            ["service", "component", "module", "app", "application"].includes(
              key,
            ),
          );
          if (hasServiceId) {
    ...
    const isMatch =
      (validJsonLines >= 2 && validRatio >= 0.5 && confidenceScore >= 0.4) ||
        ...
        const suggestions = [
          ...
          },
          {
            ...
            insertText:
              ...
              '}", "level": "${2|info,debug,warn,error|}", "message": "${3:Log message}", "service": "${4:service-name}"}',
            insertTextRules:
              monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
