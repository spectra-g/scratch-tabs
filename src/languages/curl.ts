 ...
 */
export class CurlLanguageDetector extends BaseLanguageDetector implements LanguageDetector {
...
curl -A "MyCustomClient/1.0" "https://api.example.com/search?q=test&page=1" \\
  ...
  detect(content: string): DetectionResult {
    if (!content || content.trim().length < 4) { // "curl"
      return this.noMatch();
    }
    ...
    const curlOptionsPatterns = [
      // HTTP methods
      { pattern: /\s-X\s+(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\b/gi, weight: 0.2, perMatch: 0.1 },
      // Headers
      { pattern: /\s-H\s+["'][^"':]+:[^"']+["']/g, weight: 0.15, perMatch: 0.05 },
      // Data payloads
      { pattern: /\s-d\s+(["'])/g, weight: 0.15, perMatch: 0.05 }, // Check for -d " or -d '
      { pattern: /\s--data(?:-raw|-urlencode|-binary)?\s+/g, weight: 0.15, perMatch: 0.05 },
      // Form data
      { pattern: /\s-F\s+["']?\w+=/g, weight: 0.15, perMatch: 0.05 },
      // Common single-letter flags (grouped for efficiency)
      { pattern: /\s-(?:[A-Za-z#%*@\[\]!$(),.;?_`{}~|<>^&+-=/]|\[|\])/g, weight: 0.05, perMatch: 0.01 }, // Any single letter flag
      // Common long-form flags
      { pattern: /\s--(?:request|header|form|data|user|user-agent|cookie|url|output|upload-file|insecure|cacert|cert|key|compressed|silent|show-error|verbose|include|location|head|get|http1\.1|http2)\b/gi, weight: 0.1, perMatch: 0.03 },
      ...
      { pattern: /(?<!-d\s["']|--data[^=]*=)(?<!-F\s["']\w+=)(https?:\/\/[^\s\\'"]+)/g, weight: 0.1, perMatch: 0.03 },
    ...
    const isMatch = curlCommandPattern.test(trimmedContent) && confidenceScore >= 0.45;
  ...
  registerProvider(monaco: any): void {
    ...
    if (!monaco.languages.getLanguages().some((lang: any) => lang.id === languageId)) {
      ...
      monaco.languages.setMonarchTokensProvider(languageId, {
        ...
        keywords: [ // For common cURL flags
          '-X', 'GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS',
          '-H', '-d', '-F', '--data', '--data-raw', '--data-urlencode', '--data-binary',
          '-I', '-L', '-#', '-A', '-o', '-O', '--output', '--user-agent', '--header',
          '--form', '--request', '--get', '--user', '--cookie', '--url',
          '--upload-file', '--insecure', '--cacert', '--cert', '--key',
          '--compressed', '--silent', '--show-error', '--verbose', '--include', '--location',
          '--http1.1', '--http2', '--http1.0'
        ],
        tokenizer: {
          root: [
            ...
            [/\s-[a-zA-Z#%*@\[\]!$(),.;?_`{}~|<>^&+-=\/\\](?=\s|$)/, 'keyword.flag.short'], // Single-dash flags
            [/\s--[\w-]+(?=\s|=|$)/, 'keyword.flag.long'],   // Double-dash flags

            // HTTP Methods after -X or --request
            [/(?:-X|--request)\s+(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\b/, ['keyword.flag.short', 'keyword.httpmethod']],
            ...
            [/(-d|--data|--data-raw|--data-urlencode|--data-binary)\s+/, 'keyword.flag.short'],
      ...
      monaco.editor.defineTheme(`${languageId}-theme`, {
        ...
        rules: [
          ...
          { token: 'keyword.httpmethod', foreground: '4ec9b0', fontStyle: 'bold' }, // Teal for HTTP methods
          ...
          { token: 'string.link', foreground: '569cd6', fontStyle: 'underline' }, // Blue for URLs
    ...
    monaco.languages.registerDocumentFormattingEditProvider(languageId, {
      provideDocumentFormattingEdits(model: any) {
        ...
        const finalFormatted = formattedLines.filter(line => line.trim().length > 0 || line.length === 0).join('\n');
