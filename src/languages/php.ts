 ...
 */
export class PhpLanguageDetector extends BaseLanguageDetector implements LanguageDetector {
...
`;
  ...
  detect(content: string): DetectionResult {
    ...
    const phpPatterns = [
      { pattern: /\b(class|interface|trait|enum)\s+[A-Z_][\w]*/g, weight: 0.05, perMatch: 0.02, maxMatches: 3 },
      { pattern: /\b(function|fn)\s+(?:&?\s*[a-zA-Z_][\w]*)?\s*\(/g, weight: 0.05, perMatch: 0.02, maxMatches: 5 },
      { pattern: /\$(this|self|static)\s*->\s*\w+/g, weight: 0.08, perMatch: 0.03, maxMatches: 5 },
      { pattern: /\$\w+\s*=\s*new\s+\w+/g, weight: 0.05, perMatch: 0.01, maxMatches: 3 },
      { pattern: /\b(public|private|protected|static|abstract|final)\s+(?:function|const|\$)/g, weight: 0.06, perMatch: 0.02, maxMatches: 5 },
      { pattern: /\b(namespace|use)\s+[\w\\]+;/g, weight: 0.08, perMatch: 0.03, maxMatches: 3 },
      { pattern: /::\w+/g, weight: 0.04, perMatch: 0.01, maxMatches: 5 },
      { pattern: /\b(echo|print|isset|unset|empty|require|include|die|exit)\b(?!["'=])/g, weight: 0.05, perMatch: 0.01, maxMatches: 10 },
      { pattern: /\$\w+(?:\[.*?\])?/g, weight: 0.03, perMatch: 0.005, maxMatches: 20 }, // Variables are very common
      { pattern: /\b(if|else|elseif|foreach|for|while|do|switch|case|break|continue|return|try|catch|finally|throw|match)\b/g, weight: 0.02, perMatch: 0.002, maxMatches: 10 },
      { pattern: /->\w+\s*\(/g, weight: 0.05, perMatch: 0.01, maxMatches: 5 },
      { pattern: /declare\s*\(\s*strict_types\s*=\s*1\s*\)\s*;/g, weight: 0.1, perMatch: 0.05, maxMatches: 1 },
    ];

    if (hasPhpTags) { // Only evaluate these if PHP tags were found
    ...
    if (confidenceScore < 0.8) { // Only apply anti-patterns if not already super confident
        const antiPatterns = [
          { pattern: /^\s*#include\s*<.+>/m, weight: -0.5 },
          { pattern: /^\s*import\s+(?:{[\s\S]*?}|[\w*]+)\s+from\s*['"].*?['"];?/gm, weight: -0.4 },
    ...
    if (content.includes('<?php') && content.includes('class ') && content.includes('$this->')) {
        confidenceScore += 0.1;
    }
      ...
      provideDocumentFormattingEdits(model: any) {
        ...
        const formattedLines = lines.map((line: string) => {
          ...
          if (trimmedLine.match(/^(\}|\]|\))/) ||
              (trimmedLine.match(/^(case\b|default\b)/) && indentLevel > 0 && !line.match(/^\s*(switch|{)/) ) // Avoid de-denting case after switch
             ) {
