...
jest.mock("../../components/StatusBar/LanguageStatusItems/html", () => ({
  HtmlStatusItem: () => "MockedHtmlStatusItem",
}));

import { HtmlLanguageDetector } from '../html';

describe('HtmlLanguageDetector', () => {
  let detector: HtmlLanguageDetector;

  beforeEach(() => {
    detector = new HtmlLanguageDetector();
  });
...
<body>
  ...
  describe('UI Components', () => {
    test('should return a status item component function', () => {
      ...
      expect(StatusItemComponent({ content: 'test' })).toBe('MockedHtmlStatusItem');
    });
  });
  ...
  describe('Monaco Provider Registration', () => {
    test('should register monaco provider without errors', () => {
      ...
      expect(mockMonaco.languages.register).toHaveBeenCalledWith({ id: 'html' });
      expect(mockMonaco.languages.registerDocumentFormattingEditProvider).toHaveBeenCalledWith('html', expect.any(Object));
    });
  });
}); 