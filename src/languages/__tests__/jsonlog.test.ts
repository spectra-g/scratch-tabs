import { JsonLogLanguageDetector } from '../jsonlog';

describe('JsonLogLanguageDetector', () => {
  let detector: JsonLogLanguageDetector;

  beforeEach(() => {
    detector = new JsonLogLanguageDetector();
  });
...
{"user_id": "11111", "action": "view_page", "page": "/dashboard"}`;
  ...
  describe('Monaco Provider Registration', () => {
    test('should register monaco provider without errors', () => {
      ...
      expect(mockMonaco.languages.register).toHaveBeenCalledWith({ id: 'jsonlog' });
      expect(mockMonaco.languages.setLanguageConfiguration).toHaveBeenCalledWith('jsonlog', expect.any(Object));
      expect(mockMonaco.languages.setMonarchTokensProvider).toHaveBeenCalledWith('jsonlog', expect.any(Object));
      expect(mockMonaco.editor.defineTheme).toHaveBeenCalledWith('jsonlog-dark', expect.any(Object));
      expect(mockMonaco.editor.defineTheme).toHaveBeenCalledWith('jsonlog-light', expect.any(Object));
      expect(mockMonaco.languages.registerDocumentFormattingEditProvider).toHaveBeenCalledWith('jsonlog', expect.any(Object));
      expect(mockMonaco.languages.registerCompletionItemProvider).toHaveBeenCalledWith('jsonlog', expect.any(Object));
    });
  });
}); 