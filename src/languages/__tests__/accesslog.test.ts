...
describe('AccessLogLanguageDetector', () => {
  ...
  describe('Basic Properties', () => {
    test('should have correct basic properties', () => {
      ...
      expect(detector.extensions).toEqual(['log', 'access', 'access.log', 'error.log']);
...
- - - [01/Jan/2024:10:00:02 +0000] "HEAD /health HTTP/1.1" 200 0`;
  ...
  describe('Monaco Provider Registration', () => {
    test('should register monaco provider without errors', () => {
      ...
      expect(mockMonaco.languages.register).toHaveBeenCalledWith({ id: 'accesslog' });
      expect(mockMonaco.languages.setLanguageConfiguration).toHaveBeenCalledWith('accesslog', expect.any(Object));
      expect(mockMonaco.languages.setMonarchTokensProvider).toHaveBeenCalledWith('accesslog', expect.any(Object));
      expect(mockMonaco.editor.defineTheme).toHaveBeenCalledWith('accesslog-dark', expect.any(Object));
      expect(mockMonaco.editor.defineTheme).toHaveBeenCalledWith('accesslog-light', expect.any(Object));
      expect(mockMonaco.languages.registerDocumentFormattingEditProvider).toHaveBeenCalledWith('accesslog', expect.any(Object));
      expect(mockMonaco.languages.registerCompletionItemProvider).toHaveBeenCalledWith('accesslog', expect.any(Object));
    });
  });
}); 