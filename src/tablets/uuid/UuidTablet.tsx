...
function encodeRandom(): string {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    if (i % 5 === 0) {
      ...
      index =
        ((randomBytes[Math.floor(i / 5) * 3] >> 5) |
          (randomBytes[Math.floor(i / 5) * 3 + 1] << 3)) &
        31;
    } else if (i % 5 === 2) {
      ...
      index =
        ((randomBytes[Math.floor(i / 5) * 3 + 1] >> 7) |
          (randomBytes[Math.floor(i / 5) * 3 + 2] << 1)) &
        31;
    } else {
...
function analyzeId(id: string): UuidAnalysis {
  ...
  if (uuidValidate(id)) {
    ...
    if (ver === 1) {
      ...
      const uuidTimestamp =
        ((timeHigh & 0x0fff) << 48) | (timeMid << 32) | timeLow;
...
const UuidItemComponent: React.FC<{
  ...
  const formattedId =
    format === "hyphenated" ? item.id : item.id.replace(/-/g, "");
...
export const UuidTablet: Tablet = {
    ...
    try {
      ...
      if (parsed.type === "uuid" && parsed.data) {
        ...
        if (parsed.data.analysis && parsed.data.analysis.timestamp) {
          parsed.data.analysis.timestamp = new Date(
            parsed.data.analysis.timestamp,
          );
        }

        if (parsed.data.analysis && parsed.data.analysis.ulidTimestamp) {
          parsed.data.analysis.ulidTimestamp = new Date(
            parsed.data.analysis.ulidTimestamp,
          );
        }
  ...
  render(state: UuidTabletState, onChange) {
    ...
    const updateSettings = useCallback(
      (updates: Partial<UuidTabletState["data"]["settings"]>) => {
        onChange({
          ...state,
          data: {
            ...data,
    ...
    const updateBulkGeneration = useCallback(
      (updates: Partial<UuidTabletState["data"]["bulkGeneration"]>) => {
        onChange({
          ...state,
          data: {
            ...data,
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="relative">
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  className={`transition-transform ${showExportOptions ? "rotate-180" : ""}`}
                />
              </button>
          ...
          {items.length === 0 ? (
            <div className="text-center text-secondary py-8">
              ...
              <p className="text-sm mt-1">
                Click "Generate" to create your first ID
              </p>
            </div>
          ) : (
            <div
              className={`grid gap-3 ${settings.viewMode === "grid" ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}
            >
              {items.map((item) => {
                const timestamp =
