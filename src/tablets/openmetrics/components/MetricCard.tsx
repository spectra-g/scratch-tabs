...
export const MetricCard: React.FC<MetricCardProps> = ({ metricInfo, samples, onSelect }) => {
  const getTypeColor = (type: string | undefined) => {
    switch (type) {
      case 'counter': return 'bg-blue-500/20 text-blue-400 border-blue-500/30';
      case 'gauge': return 'bg-green-500/20 text-green-400 border-green-500/30';
      case 'histogram': return 'bg-purple-500/20 text-purple-400 border-purple-500/30';
      case 'summary': return 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30';
  ...
  return (
    <div 
      ...
      onClick={onSelect}
    >
      <div className="font-medium text-sm mb-2 truncate" title={metricInfo.name}>
        {metricInfo.name}
      </div>
      
      {metricInfo.help && (
        <div className="text-xs text-gray-400 mb-3 line-clamp-2" title={metricInfo.help}>
          {metricInfo.help}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        {metricInfo.type === 'counter' || metricInfo.type === 'gauge' ? (
          <div className="text-center p-1 bg-black/20 rounded">
            ...
            <div className="text-sm font-medium">{formatNumber(samples[0]?.value || 0)}</div>
          </div>
        ) : (
          <div className="text-center p-1 bg-black/20 rounded">
