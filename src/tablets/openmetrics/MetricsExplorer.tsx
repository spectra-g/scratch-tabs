    ...
    return parsedMetrics.filter(
      (metric) =>
        ...
        metric.samples.some((sample) =>
          Object.entries(sample.labels).some(
            ([key, value]) =>
        ...
        {isExpanded && (
          <div className="pl-6 mt-1 space-y-1">
            ...
            {uniqueLabels.length > 0 ? (
              <div className="space-y-1">
                {uniqueLabels.map((labelKey) =>
                  renderLabelGroup(metric, labelKey),
                )}
              </div>
            ) : (
              <div
...
const TypeBadge: React.FC<{ type: string | undefined }> = ({ type }) => {
  ...
  return (
    <span
      className={`ml-2 text-xs px-1.5 py-0.5 rounded ${getTypeColor(type)}`}
    >
      {type}
    </span>
  );
