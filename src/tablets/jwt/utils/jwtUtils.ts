...
export function decodeJwt(token: string): DecodedJwt {
  try {
    ...
    if (parts.length < 2) {
      warning =
        "Incomplete JWT format. Expected at least header and payload parts.";
...
export async function verifyJwt(
  ...
  try {
    ...
    if (!algorithm) {
      return {
        isValid: false,
        error: "No algorithm specified in token header",
      };
    }
    ...
    if (algorithm.startsWith("HS")) {
      ...
      try {
        ...
        if (isPem || keyType === "pem") {
          ...
          return {
            isValid: false,
            error: "Invalid key type for asymmetric algorithm",
          };
        }
      } catch (error) {
        return {
    ...
    try {
      ...
      if (errorMessage.includes('"exp" claim timestamp check failed')) {
        return {
          isValid: true,
          warning: "Note: The token has expired, but the signature is valid.",
        };
      }
...
export async function generateKeyPair(
  ...
  try {
    ...
    if (algorithm.startsWith("RS") || algorithm.startsWith("PS")) {
      ...
      throw new Error(
        `Unsupported algorithm for key pair generation: ${algorithm}`,
      );
    }
    ...
    throw new Error(
      `Key pair generation failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export function isPemFormat(str: string): boolean {
  return /^-----BEGIN (PRIVATE|PUBLIC) KEY-----[\s\S]*-----END (PRIVATE|PUBLIC) KEY-----$/m.test(
    str.trim(),
  );
}
 ...
 */
export function createJwtFromParts(
  header: string,
  payload: string,
  signature: string,
): string {
