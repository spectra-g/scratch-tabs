  ...
  try {
    ...
    Object.entries(snippets).forEach(([key, snippet]: [string, any]) => {
      try {
        ...
        errors.push(
          `Snippet "${key}": ${error instanceof Error ? error.message : "Invalid format"}`,
        );
      }
    });
  } catch (error) {
  ...
  try {
    ...
    sections.forEach((section, index) => {
      try {
          ...
          items.push({
            title:
              generatedTitle.length > 100
                ? generatedTitle.substring(0, 100) + "..."
                : generatedTitle,
            content,
            contentType: "plaintext",
            labels: ["markdown-import"],
        ...
        errors.push(
          `Section ${index + 1}: ${error instanceof Error ? error.message : "Failed to parse"}`,
        );
      }
    });
  } catch (error) {
  ...
  switch (source) {
    case "terminal-history":
      return {
        ...
        placeholder:
          "Paste your terminal history here...\nExample:\n998 chmod +x inspect_folder.sh\n999 git status\n1000 npm install",
        acceptsFiles: false,
      };
    case "vscode-snippets":
      return {
    ...
    case "markdown-notes":
      return {
        name: "Markdown Notes",
        description:
          "Import notes from markdown files separated by horizontal rules (---)",
        placeholder:
