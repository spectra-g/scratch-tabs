...
1000 LC_ALL=C od -c -tx1 hooks/useEditorScrollManager.ts | head -n 20`;
      ...
      const viCommands = result.items.filter(
        (item) => item.content === "vi inspect_folder.sh",
      );
      const chmodCommands = result.items.filter(
        (item) => item.content === "chmod +x inspect_folder.sh",
      );
      const inspectCommands = result.items.filter(
        (item) => item.content === "./inspect_folder.sh",
      );
      const rmCommands = result.items.filter(
        (item) => item.content === "rm inspect_folder.sh",
      );
