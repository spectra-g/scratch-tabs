...
export const VaultTablet: Tablet = {
  ...
  deserializeState(json: string): TabletState {
    try {
      ...
      if (parsed.type === "vault" && parsed.data) {
        ...
        const isOldFormat = parsed.data.viewMode === "card" || parsed.data.viewMode === "list" || !parsed.data.hasOwnProperty("scratchpadContent");
  ...
  render(state: VaultTabletState, onChange) {
    ...
    const [deleteConfirmItemId, setDeleteConfirmItemId] = useState<
      string | null
    >(null);
    ...
    const handleFilterByContentType = useCallback(
        ...
        onChange({
          ...
          data: {
            ...
            activeFilters: {
              ...state.data.activeFilters,
              contentType:
                state.data.activeFilters.contentType === contentType
                  ? null
                  : contentType,
            },
          },
    ...
    const handleImportItemsConfirm = useCallback(
      (newItems: VaultItem[]) => {
    ...
    const handleCopyContent = useCallback(
      (id: string) => {
        ...
        if (
          (state.data.sortOrder === "lastUsed" ||
            state.data.sortOrder === "usageCount") &&
      ...
      return (
        ...
        >
          ...
          <div className="flex items-center gap-1 mr-3 flex-shrink-0">
            ...
            <div className="w-8 text-left text-gray-400 text-sm">
              {item.usageCount}
            </div>
          </div>
