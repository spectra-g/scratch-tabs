...
describe('useResizeHandler', () => {
  ...
  const createShape = (type: string, x: number, y: number, width: number, height: number): Shape => ({
    id: 'test-shape',
    type,
    x,
    y,
  ...
  describe('detectResizeHandle', () => {
    it('should detect corner handles for rectangle', () => {
      const { result } = renderHook(() => useResizeHandler({ onUpdateShape: mockOnUpdateShape }));
      
      const rect = createShape('rectangle', 100, 100, 100, 100);
      
      // Test corner handles
      expect(result.current.detectResizeHandle(rect, { x: 95, y: 95 })).toBe('nw');
      expect(result.current.detectResizeHandle(rect, { x: 205, y: 95 })).toBe('ne');
      expect(result.current.detectResizeHandle(rect, { x: 205, y: 205 })).toBe('se');
      expect(result.current.detectResizeHandle(rect, { x: 95, y: 205 })).toBe('sw');
    });
    ...
    it('should detect edge handles for rectangle', () => {
      ...
      // Test edge handles
      expect(result.current.detectResizeHandle(rect, { x: 150, y: 95 })).toBe('n');
      expect(result.current.detectResizeHandle(rect, { x: 205, y: 150 })).toBe('e');
      expect(result.current.detectResizeHandle(rect, { x: 150, y: 205 })).toBe('s');
      expect(result.current.detectResizeHandle(rect, { x: 95, y: 150 })).toBe('w');
    });
    ...
    it('should return null for line shapes', () => {
      ...
      expect(result.current.detectResizeHandle(line, { x: 150, y: 150 })).toBeNull();
    });
    ...
    it('should return null when not near any handle', () => {
      ...
      expect(result.current.detectResizeHandle(rect, { x: 300, y: 300 })).toBeNull();
    });
  });
  ...
  describe('resize operations', () => {
    ...
    it('should apply grid snapping when enabled', () => {
      const { result } = renderHook(() => 
        useResizeHandler({ onUpdateShape: mockOnUpdateShape, gridSnappingEnabled: true })
      );
