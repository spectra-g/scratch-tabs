...
describe('useArrowTipHandler', () => {
  ...
  const createLineShape = (x1: number, y1: number, x2: number, y2: number, arrowTipStart?: string, arrowTipEnd?: string): Shape => ({
    id: 'test-line',
    type: 'line',
  ...
  describe('detectArrowTipClick', () => {
    it('should return false for non-line shapes', () => {
      const { result } = renderHook(() => useArrowTipHandler({ onUpdateShape: mockOnUpdateShape }));
      ...
      const arrowTipState = result.current.detectArrowTipClick(rect, { x: 100, y: 100 });
    ...
    it('should detect arrow tip clicks even when no arrow tips are set (allows cycling)', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(line, { x: 200, y: 200 });
    ...
    it('should detect start arrow tip click', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(line, { x: 105, y: 105 });
    ...
    it('should detect end arrow tip click', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(line, { x: 195, y: 195 });
    ...
    it('should detect both start and end arrow tips', () => {
      ...
      const line = createLineShape(100, 100, 200, 200, 'simple', 'filled-triangle');
      
      // Test start tip
      const startArrowTipState = result.current.detectArrowTipClick(line, { x: 105, y: 105 });
      expect(startArrowTipState.isArrowTipClick).toBe(true);
      expect(startArrowTipState.arrowTipMode).toBe('resize-start');
      
      // Test end tip
      const endArrowTipState = result.current.detectArrowTipClick(line, { x: 195, y: 195 });
    ...
    it('should not detect arrow tip click when too far from tips', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(line, { x: 150, y: 150 });
    ...
    it('should use adaptive threshold based on line length', () => {
      ...
      const shortLineState = result.current.detectArrowTipClick(shortLine, { x: 155, y: 100 });
      expect(shortLineState.isArrowTipClick).toBe(true); // Within 5px threshold
      
      // Test with point that should be too far
      const farShortLineState = result.current.detectArrowTipClick(shortLine, { x: 160, y: 100 });
      ...
      const longLineState = result.current.detectArrowTipClick(longLine, { x: 310, y: 100 });
      ...
      const closerLongLineState = result.current.detectArrowTipClick(longLine, { x: 295, y: 100 });
