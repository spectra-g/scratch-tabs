...
export const useArrowTipHandler = ({
  ...
  const detectArrowTipClick = useCallback((shape: Shape, mousePoint: Point): ArrowTipState => {
    if (shape.type !== 'line') {
  ...
  const handleArrowTipClick = useCallback((shape: Shape, arrowTipMode: 'resize-start' | 'resize-end') => {
