...
export const useShapeSnapEngineV2 = (
  ...
  const selectionManager = useMemo(() => new SelectionManager(state, onChange), [state, onChange]);
  ...
  const updateShapeLabel = useCallback((shapeId: string, label: string) => {
    const command = new UpdateShapeCommand(getCurrentState, onChange, shapeId, { label: label || undefined });
    commandManager.executeCommand(command);
  }, [getCurrentState, onChange, commandManager]);

  // Update a shape with new properties
  const updateShape = useCallback((shapeId: string, updates: Partial<Shape>) => {
    const command = new UpdateShapeCommand(getCurrentState, onChange, shapeId, updates);
    commandManager.executeCommand(command);
  }, [getCurrentState, onChange, commandManager]);

  // Delete a shape by ID
  const deleteShape = useCallback((shapeId: string) => {
    const command = new DeleteShapeCommand(getCurrentState, onChange, shapeId);
    commandManager.executeCommand(command);
  }, [getCurrentState, onChange, commandManager]);
  ...
  const deleteSelectedShapes = useCallback(() => {
    ...
    const command = new DeleteSelectedShapesCommand(getCurrentState, onChange, selectedIds);
    commandManager.executeCommand(command);
  ...
  const pasteShapes = useCallback(() => {
    ...
    const pastedShapes = state.clipboard.map(shape => {
      ...
      } else if (shape.type === 'arrow' && (shape as any).from && (shape as any).to) {
        return {
          ...newShape,
  ...
  const detectAndAddShape = useCallback((points: Point[]) => {
    ...
    if (detectedGeometry) {
      const strokeColor = getCurrentState().canvas.mode === 'dark' ? '#ffffff' : '#000000';
  ...
  const clearCanvas = useCallback(() => {
    ...
    if (allShapeIds.length > 0) {
      const command = new DeleteSelectedShapesCommand(getCurrentState, onChange, allShapeIds);
      commandManager.executeCommand(command);
    }
  ...
  const cycleFontSize = useCallback(() => {
    ...
    textShapes.forEach(shape => {
      const command = new UpdateShapeCommand(getCurrentState, onChange, shape.id, { fontSize: newFontSize });
      commandManager.executeCommand(command);
    });
  ...
  // Move shape by delta
  const moveShape = useCallback((shapeId: string, delta: Point) => {
    const command = new MoveShapeCommand(getCurrentState, onChange, shapeId, delta);
    commandManager.executeCommand(command);
  }, [getCurrentState, onChange, commandManager]);
  ...
  const selectInArea = useCallback((startPoint: Point, endPoint: Point) => {
    selectionManager.selectInArea(startPoint, endPoint);
  }, [selectionManager]);

  const getShapesAtPoint = useCallback((point: Point) => {
    return selectionManager.getShapesAtPoint(point);
  }, [selectionManager]);

  const getTopShapeAtPoint = useCallback((point: Point) => {
    return selectionManager.getTopShapeAtPoint(point);
  }, [selectionManager]);

  const handleShapeClick = useCallback((shape: Shape, point: Point, modifiers: { ctrl: boolean; shift: boolean; alt: boolean }) => {
    selectionManager.handleShapeClick(shape, point, modifiers);
  }, [selectionManager]);

  const handleCanvasClick = useCallback((point: Point, modifiers: { ctrl: boolean; shift: boolean; alt: boolean }) => {
  ...
  return {
    ...
    redo,
      ...
      // State queries
  get canUndo() { return commandManager.canUndo(); },
  get canRedo() { return commandManager.canRedo(); },
