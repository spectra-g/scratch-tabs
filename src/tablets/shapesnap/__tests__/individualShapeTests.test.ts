...
describe("Test Summary", () => {
  test("should display comprehensive accuracy summary", () => {
    ...
    Object.entries(shapeStats).forEach(([expectedShape, stats]) => {
      ...
      if (Object.keys(stats.misclassifications).length > 0) {
        const misclassList = Object.entries(stats.misclassifications)
          .map(
            ([detected, count]) =>
              `${count} incorrectly detected as ${detected}`,
          )
          .join(", ");
    ...
    const overallAccuracy =
      totalTests > 0 ? ((totalCorrect / totalTests) * 100).toFixed(1) : "0.0";
    console.log("\n" + "-".repeat(80));
    console.log(
    ...
    Object.entries(shapeStats).forEach(([expectedShape, stats]) => {
      ...
      if (accuracy < 50) {
        console.log(
          `  ⚠️  ${expectedShape.toUpperCase()} has poor accuracy (${accuracy}%)`,
        );
      } else if (accuracy < 80) {
        console.log(
          `  ⚡ ${expectedShape.toUpperCase()} needs improvement (${accuracy}%)`,
        );
      } else {
        console.log(
          `  ✅ ${expectedShape.toUpperCase()} performing well (${accuracy}%)`,
        );
      }
    });
