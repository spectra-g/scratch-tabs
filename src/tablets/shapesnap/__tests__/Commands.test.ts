...
describe('Commands', () => {
  ...
  describe('UpdateShapeCommand', () => {
    ...
    it('should update shape properties', () => {
      ...
      const command = new UpdateShapeCommand(getCurrentState, mockOnChange, 'shape1', updates);
    ...
    it('should undo shape updates', () => {
      ...
      expect(mockOnChange).toHaveBeenCalled();
      const newState = mockOnChange.mock.calls[0][0];
      const revertedShape = newState.shapes.find((s: Shape) => s.id === 'shape1');
    ...
    it('should handle non-existent shape gracefully', () => {
      const command = new UpdateShapeCommand(getCurrentState, mockOnChange, 'nonexistent', { x: 50 });
  ...
  describe('DeleteShapeCommand', () => {
    ...
    it('should delete a shape from the canvas', () => {
      const command = new DeleteShapeCommand(getCurrentState, mockOnChange, 'shape1');

      command.execute();

      expect(mockOnChange).toHaveBeenCalled();
      const newState = mockOnChange.mock.calls[0][0];
      expect(newState.shapes.find((s: Shape) => s.id === 'shape1')).toBeUndefined();
      expect(newState.shapes.length).toBe(0);
    });
    ...
    it('should undo shape deletion', () => {
      ...
      expect(mockOnChange).toHaveBeenCalled();
      const newState = mockOnChange.mock.calls[0][0];
      const restoredShape = newState.shapes.find((s: Shape) => s.id === 'shape1');
  ...
  describe('DeleteSelectedShapesCommand', () => {
    ...
    it('should delete multiple selected shapes', () => {
      const command = new DeleteSelectedShapesCommand(getCurrentState, mockOnChange, ['shape1', 'shape3']);
    ...
    it('should undo multiple shape deletion', () => {
      ...
      expect(newState.shapes.map((s: Shape) => s.id).sort()).toEqual(['shape1', 'shape2', 'shape3']);
    });
  });
  ...
  describe('MoveShapeCommand', () => {
    ...
    it('should move a rectangle shape', () => {
      ...
      const command = new MoveShapeCommand(getCurrentState, mockOnChange, 'shape1', delta);
    ...
    it('should move a line shape', () => {
      ...
      const command = new MoveShapeCommand(getCurrentState, mockOnChange, 'line1', delta);

      command.execute();

      expect(mockOnChange).toHaveBeenCalled();
      const newState = mockOnChange.mock.calls[0][0];
      const movedShape = newState.shapes.find((s: Shape) => s.id === 'line1') as any;
  ...
  describe('CommandManager', () => {
    ...
    it('should execute commands in order', () => {
      ...
      const command1 = new AddShapeCommand(getCurrentState, mockOnChange, shape1);
      const command2 = new AddShapeCommand(getCurrentState, mockOnChange, shape2);
