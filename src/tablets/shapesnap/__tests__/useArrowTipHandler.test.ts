...
describe('useArrowTipHandler', () => {
  ...
  describe('detectArrowTipClick', () => {
    it('should detect start arrow tip click', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(mockLineShape, startPoint);
    ...
    it('should detect end arrow tip click', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(mockLineShape, endPoint);
    ...
    it('should not detect arrow tip click for non-line shapes', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(rectShape, point);
    ...
    it('should not detect arrow tip click when too far from endpoints', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(mockLineShape, middlePoint);
    ...
    it('should handle lines with insufficient points', () => {
      ...
      const arrowTipState = result.current.detectArrowTipClick(invalidLineShape, point);
  ...
  describe('threshold detection', () => {
    it('should use percentage-based threshold for long lines', () => {
      ...
      const startArrowTipState = result.current.detectArrowTipClick(longLineShape, nearStart);
      ...
      const endArrowTipState = result.current.detectArrowTipClick(longLineShape, nearEnd);
    ...
    it('should use fixed threshold for short lines', () => {
      ...
      const startArrowTipState = result.current.detectArrowTipClick(shortLineShape, nearStart);
      expect(startArrowTipState.isArrowTipClick).toBe(true);
      expect(startArrowTipState.arrowTipMode).toBe('resize-start');

      // Test near end point (should be resize-end)
      const nearEnd = { x: 8, y: 0 }; // Within 15px threshold
      const endArrowTipState = result.current.detectArrowTipClick(shortLineShape, nearEnd);
      expect(endArrowTipState.isArrowTipClick).toBe(true);
      expect(endArrowTipState.arrowTipMode).toBe('resize-end');
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle lines with only start arrow tip', () => {
      ...
      const startPoint = { x: 105, y: 105 };
      const arrowTipState = result.current.detectArrowTipClick(lineWithStartOnly, startPoint);
