...
export const UrlInput: React.FC<UrlInputProps> = ({
  ...
  const hasErrors = warnings.some(w => w.type === 'error' && w.component === 'full');
    ...
    <div className="mb-4">
      ...
      <div className="relative">
        ...
        <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center space-x-1">
          ...
          <button
            ...
            disabled={!url}
            className={`p-1 ${
              url ? 'text-gray-400 hover:text-gray-200 hover:bg-gray-700' : 'text-gray-600 cursor-not-allowed'
            } rounded`}
      ...
      {hasErrors && (
        <div className="mt-1 text-xs text-red-400">
          {warnings.find(w => w.type === 'error' && w.component === 'full')?.message}
        </div>
      )}
    </div>
  );
