...
export const UrlParserTablet: Tablet = {
  id: 'urlparser',
  label: 'URL Parser',
  keywords: ['url', 'uri', 'parser', 'analyzer', 'web', 'http', 'https', 'domain', 'query', 'fragment'],
  ...
  render(state: TabletState, onChange: (state: TabletState) => void) {
    ...
    const handleUpdateUrl = useCallback((newUrl: string) => {
      ...
      onChange({
        ...
        data: {
          ...
          history: newUrl && !typedState.data.history.includes(newUrl)
    ...
    const handleUpdateComponent = useCallback((value: string, component: keyof UrlComponents) => {
    ...
    const handleUpdateQueryParams = useCallback((params: Record<string, string>) => {
    ...
    const handleToggleEncoding = useCallback(() => {
      onChange({
        ...
        data: {
          ...typedState.data,
          viewMode: typedState.data.viewMode === 'encoded' ? 'decoded' : 'encoded'
        }
      });
    }, [typedState, onChange]);
