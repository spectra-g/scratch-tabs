 ...
 */
export function parseUrl(urlString: string): { components: UrlComponents; warnings: UrlWarning[] } {
  ...
  if (urlString.includes('\\')) {
    warnings.push({
      ...
      description: 'Backslashes (\\) are not standard in URLs. Browsers may convert them to forward slashes, but this can lead to unexpected behavior.',
      suggestion: 'Replace backslashes with forward slashes (/).'
    });
  }
  
  // Check for encoded slashes
  if (urlString.includes('%2F') || urlString.includes('%2f') || 
      urlString.includes('%5C') || urlString.includes('%5c')) {
    warnings.push({
      type: 'warning',
      component: 'path',
      message: 'URL contains encoded slashes',
      description: 'Encoded slashes (%2F, %5C) can be used to bypass security filters or cause path traversal issues.',
  ...
  if (urlString.includes('%00')) {
    warnings.push({
      ...
      description: 'Null bytes (%00) can be used to trick systems into truncating strings and are often used in attacks.',
  ...
  if (atSymbolCount > 1) {
    warnings.push({
      ...
      description: 'Multiple @ symbols can cause confusion about which part is the host. Browsers typically use the last @ to determine the host.',
  ...
  try {
    ...
    if (!normalizedUrl.includes('://')) {
      ...
      warnings.push({
        ...
        suggestion: 'Add an explicit scheme (e.g., http://, https://) for clarity.'
      });
    }
    ...
    if (url.username || url.password) {
      warnings.push({
        ...
        description: 'Including credentials in URLs is a security risk. They may be logged or leaked in browser history.',
        suggestion: 'Use secure authentication methods instead of embedding credentials in URLs.'
      });
    }
    ...
    if (url.hostname !== url.hostname.toLowerCase()) {
      warnings.push({
        ...
        description: 'Domain contains mixed case characters which may indicate a spoofing attempt.',
    ...
    if (url.hostname.includes('xn--')) {
      try {
        ...
        warnings.push({
          ...
          suggestion: 'Verify this is the intended domain as IDNs can be used for homograph attacks.'
        });
      } catch (e) {
        ...
        warnings.push({
          ...
          description: 'This domain uses Punycode encoding which could be used for deception.',
    ...
    if (url.hostname.includes(']') && url.port) {
      warnings.push({
        ...
        description: 'IPv6 addresses with ports can be parsed inconsistently across systems.',
    ...
    warnings.push({
      ...
      description: 'The URL could not be parsed by the standard URL constructor.',
  ...
  if (components.port) {
    ...
    if (isNaN(port) || port < 1 || port > 65535) {
      ...
      warnings.push({
        ...
        description: 'Port 80 is the default for HTTP and doesn\'t need to be specified.',
      ...
      warnings.push({
        ...
        description: 'Port 443 is the default for HTTPS and doesn\'t need to be specified.',
        suggestion: 'Consider removing the explicit port for cleaner URLs.'
      });
    }
  }
  ...
  if (components.scheme && !['http', 'https', 'ftp', 'file', 'data', 'mailto', 'tel', 'sms'].includes(components.scheme)) {
    warnings.push({
      type: 'warning',
      component: 'scheme',
  ...
  if (components.query) {
    if (components.query.includes(';')) {
      warnings.push({
        ...
        description: 'The query string uses semicolons (;) instead of ampersands (&) as separators.',
        suggestion: 'Consider using standard & separators for better compatibility.'
      });
    }
    ...
    if (paramNames.length !== uniqueParamNames.size) {
      warnings.push({
        ...
        description: 'The URL contains duplicate query parameter names. Different systems may handle these inconsistently.',
  ...
  if (components.fragment && components.fragment.includes('=')) {
    warnings.push({
      ...
      description: 'The fragment appears to contain key-value pairs, which may indicate confusion between fragment and query components.',
      suggestion: 'Consider moving these parameters to the query string if they\'re meant to be sent to the server.'
    });
  }
 ...
 */
export function encodeComponent(value: string, component: keyof UrlComponents): string {
  if (!value) return '';
 ...
 */
export function decodeComponent(value: string, component: keyof UrlComponents): string {
  if (!value) return '';
 ...
 */
export function compareUrlParsing(urlString: string): Record<string, UrlComponents> {
 ...
 */
export function getSuspiciousUrlExamples(): { url: string; description: string }[] {
  return [
    {
