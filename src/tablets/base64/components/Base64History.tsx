...
export const Base64History: React.FC<Base64HistoryProps> = ({
  ...
  return (
    <motion.div
      ...
      className="border-t border-base bg-element overflow-hidden"
    >
      <div className="p-3">
        ...
        {history.length > 0 && (
          <div className="relative mb-3">
            <input
              ...
              className="w-full bg-input border border-base rounded-md pl-9 pr-9 py-1.5 text-sm text-main placeholder-muted focus:outline-none focus:border-focus transition-colors"
            />
            <Search
              size={14}
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar pr-1">
          {filteredHistory.length === 0 ? (
            ...
            filteredHistory.map((item) => (
              <div
                key={item.id}
                className="bg-surface-secondary/50 rounded-lg p-2 hover:bg-surface-secondary transition-colors"
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center space-x-2">
                    {item.action === "encode" ? (
                      ...
                      <ArrowDownRight size={14} className="text-info" />
                    )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="text-xs bg-element rounded p-1.5 overflow-hidden">
                    <div className="text-secondary mb-1 flex justify-between">
                      <span>
                        Input ({formatFileSize(new Blob([item.input]).size)})
                      </span>
                      <span className="text-muted">{item.encoding}</span>
                    </div>
                    <div className="font-mono text-main truncate">
