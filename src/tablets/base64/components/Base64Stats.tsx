...
export const Base64Stats: React.FC<Base64StatsProps> = ({ stats, mode }) => {
  ...
  const isEncoding = mode === 'encode' || (mode === 'line-by-line' && stats.ratio > 1);
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div className="bg-gray-700/30 rounded p-1.5">
          ...
          <div className="text-gray-200 font-medium">{formatFileSize(stats.originalSize)}</div>
        </div>
        <div className="bg-gray-700/30 rounded p-1.5">
          <div className="text-gray-500 mb-0.5">{isEncoding ? 'Encoded' : 'Decoded'} Size</div>
          <div className="text-gray-200 font-medium">{formatFileSize(stats.encodedSize)}</div>
        </div>
        <div className="bg-gray-700/30 rounded p-1.5">
          <div className="text-gray-500 mb-0.5">Size Ratio</div>
          <div className="text-gray-200 font-medium">{stats.ratio.toFixed(2)}x</div>
        </div>
        <div className="bg-gray-700/30 rounded p-1.5">
          <div className="text-gray-500 mb-0.5">Size Change</div>
          <div className={`font-medium flex items-center ${isIncrease ? 'text-yellow-400' : 'text-green-400'}`}>
