...
const Base64TabletUI: React.FC<{
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const tabletContainer = document.querySelector(
  ...
  const setMode = useCallback(
    (newMode: "encode" | "decode" | "line-by-line") => {
      onChange({
        ...state,
        data: {
          ...state.data,
  ...
  const setSelectedEncoding = useCallback(
    (encoding: string) => {
      onChange({
        ...state,
        data: {
  ...
  const setWrapOutput = useCallback(
    (wrap: boolean) => {
      onChange({
        ...state,
        data: {
  ...
  const setPreserveNewlines = useCallback(
    (preserve: boolean) => {
      onChange({
        ...state,
        data: {
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <div
        className={`flex-1 p-3 flex ${layout === "horizontal" ? "flex-row" : "flex-col"} gap-3 overflow-auto custom-scrollbar`}
      >
        {/* Input */}
        <Base64Input
          value={input}
        ...
        {/* Output */}
        <Base64Output value={output} isValid={isValidBase64} mode={mode} />
      </div>
