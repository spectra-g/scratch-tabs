...
function encodeUtf7(input: string): string {
  ...
  return (
    "+" +
    standard.replace(/\+/g, "+-").replace(/\//g, ",").replace(/=/g, "") +
    "-"
  );
}
...
export const base64Formats: Base64Format[] = [
  ...
  },
  {
    ...
    description:
      'URL and filename safe Base64 variant that uses "-" and "_" instead of "+" and "/"',
  ...
  },
  {
    ...
    name: "MIME (RFC 2045)",
    description:
      "MIME Base64 encoding with optional line wrapping at 76 characters",
    encode: encodeMime,
    decode: decodeMime,
