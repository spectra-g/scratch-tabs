...
export const PomodoroTimeline: React.FC<PomodoroTimelineProps> = ({ sessions }) => {
        ...
        <div className="space-y-3">
          {sortedSessions.map((session, index) => (
            <div key={index} className="flex items-start ml-4 pl-6 relative">
              ...
              } ${!session.completed ? 'opacity-50' : ''}`}></div>
              
              {/* Session content */}
              <div className={`flex-1 p-3 rounded-md ${
                session.type === 'focus' ? 'bg-blue-500/10 border border-blue-500/20' :
                session.type === 'shortBreak' ? 'bg-green-500/10 border border-green-500/20' :
                'bg-purple-500/10 border border-purple-500/20'
              }`}>
                <div className="flex justify-between items-start">
                  <div>
                    <div className={`text-sm font-medium ${
                      session.type === 'focus' ? 'text-blue-400' :
                      session.type === 'shortBreak' ? 'text-green-400' :
                      'text-purple-400'
                    }`}>
