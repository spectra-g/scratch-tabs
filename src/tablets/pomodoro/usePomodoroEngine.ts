...
export const usePomodoroEngine = (
  ...
  onChange: (newState: PomodoroState) => void,
) => {
  ...
  useEffect(() => {
    if (pendingStateRef.current) {
      ...
      const isTimerUpdate =
        pendingStateRef.current.status === "running" &&
        state.status === "running" &&
        pendingStateRef.current.currentSession.timeRemaining !==
    ...
    timerRef.current = window.setInterval(() => {
      setState((prevState) => {
        if (
          ...
          prevState.currentSession.startTime === 0
        ) {
          return prevState;
        }
        ...
        const elapsed = Math.floor(
          (now - prevState.currentSession.startTime) / 1000,
        );
        const timeRemaining = Math.max(
          0,
          prevState.currentSession.duration - elapsed,
        );
            ...
            prevState.status === "paused"
              ? now -
                (prevState.currentSession.duration -
                  prevState.currentSession.timeRemaining) *
                  1000
              : now,
        },
      };
  ...
  const handleSettingsChange = useCallback(
    (newSettings: PomodoroSettingsType) => {
      setState((prevState) => {
        const newDuration =
          prevState.currentSession.type === "focus"
