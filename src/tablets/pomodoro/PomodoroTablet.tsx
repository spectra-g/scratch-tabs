        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-md transition-colors ${status === "running" ? "bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30" : "bg-green-500/20 text-green-400 hover:bg-green-500/30"}`}
...
const PomodoroTabletWrapper: React.FC<{
  ...
  const tabletInstanceId = React.useMemo(
    () => `pomodoro-${crypto.randomUUID()}`,
    [],
  );
  return (
    <PomodoroTabletUI
...
export const PomodoroTablet: Tablet = {
    ...
    try {
      ...
      if (parsed.type === "pomodoro" && parsed.data) {
        ...
        const data = {
          ...
          currentSession: {
            ...defaultState.data.currentSession,
            ...parsed.data.currentSession,
          },
          todayStats: {
            ...defaultState.data.todayStats,
            ...parsed.data.todayStats,
          },
          sessions: Array.isArray(parsed.data.sessions)
  ...
  render(state: TabletState, onChange) {
    ...
    return (
      <PomodoroTabletWrapper
        state={pomodoroState}
        onChange={onChange as (newState: PomodoroTabletState) => void}
      />
    );
  },
};
