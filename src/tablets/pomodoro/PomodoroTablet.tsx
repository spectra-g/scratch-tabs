  ...
  deserializeState(json: string): TabletState {
    try {
      ...
      if (parsed.type === 'pomodoro' && parsed.data) {
        ...
        // Ensure current session has all required properties
        const currentSession = {
          type: data.currentSession?.type || 'focus',
          duration: data.currentSession?.duration || settings.focusDuration * 60,
          timeRemaining: data.currentSession?.timeRemaining || settings.focusDuration * 60,
          startTime: data.currentSession?.startTime || 0,
          pauseTime: data.currentSession?.pauseTime || 0,
          totalPausedTime: data.currentSession?.totalPausedTime || 0
        };
        
        // Ensure todayStats has all required properties
        const todayStats = {
          focusCompleted: data.todayStats?.focusCompleted || 0,
          shortBreakCompleted: data.todayStats?.shortBreakCompleted || 0,
          longBreakCompleted: data.todayStats?.longBreakCompleted || 0,
          totalFocusTime: data.todayStats?.totalFocusTime || 0,
          currentStreak: data.todayStats?.currentStreak || 0,
          bestStreak: data.todayStats?.bestStreak || 0
        };
        ...
        return {
          ...
          data: {
            ...
            sessions,
            todayStats,
            activeView: data.activeView || 'timer',
