...
interface PromptListProps {
  ...
  onCreatePrompt: (prompt: Omit<Prompt, 'id' | 'createdAt' | 'lastModified' | 'usageCount'>) => Prompt;
  ...
  onSortChange: (sortBy: 'title' | 'createdAt' | 'lastModified' | 'usageCount') => void;
...
export const PromptList: React.FC<PromptListProps> = ({
  ...
  const getTagsForPrompt = (prompt: Prompt) => {
    return prompt.tags.map(tagId => tags.find(tag => tag.id === tagId)).filter(Boolean) as Tag[];
  };
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-20 min-w-[180px]">
                    <div className="py-1">
                      ...
                      <button
                        className={`flex items-center justify-between w-full px-4 py-2 text-sm text-left hover:bg-gray-700 transition-colors ${
                          sortBy === 'usageCount' ? 'text-blue-400' : 'text-gray-300'
                        }`}
                        onClick={() => {
              ...
              >
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <h3 className="font-medium text-gray-200 truncate">{prompt.title}</h3>
                    <div className="flex items-center space-x-1">
                      <button
                      ...
                      >
                        <Star size={14} className={prompt.isFavorite ? 'fill-yellow-400' : ''} />
                      </button>
                      <button
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-500 mt-1">
                    ...
                    <span>{prompt.usageCount} {prompt.usageCount === 1 ? 'use' : 'uses'}</span>
                  </div>
                </div>
              </div>
            ))}
