...
export const PromptManagerTablet: Tablet = {
    ...
    try {
      ...
      if (parsed.type === "promptmanager" && parsed.data) {
        ...
        data.settings = {
          ...
          viewMode: data.settings?.viewMode || "list",
        };
        ...
        data.ui = {
          ...
          selectedTags: Array.isArray(data.ui?.selectedTags)
            ? data.ui.selectedTags
            : [],
          showFavoritesOnly: data.ui?.showFavoritesOnly || false,
          showFiltersPanel: data.ui?.showFiltersPanel ?? false,
        };
  ...
  render(state: PromptManagerTabletState, onChange) {
    ...
    const updateSettings = (
      newSettings: Partial<PromptManagerData["settings"]>,
    ) => {
      onChange({
        ...state,
        data: {
    ...
    const deletePrompt = (id: string) => {
      ...
      const newUI =
        data.ui.selectedPromptId === id
          ? {
              selectedPromptId:
                updatedPrompts.length > 0 ? updatedPrompts[0].id : null,
            }
          : {};
    ...
    const createWorkflow = (
      workflow: Omit<Workflow, "id" | "createdAt" | "lastModified">,
    ) => {
    ...
    const deleteWorkflow = (id: string) => {
      const updatedWorkflows = (data.workflows || []).filter(
        (workflow) => workflow.id !== id,
      );

      // If the deleted workflow was selected, clear selection
      const newUI =
        data.ui.selectedWorkflowId === id
          ? {
              selectedWorkflowId:
    ...
    const deleteTemplate = (id: string) => {
      ...
      const updatedTemplates = data.templates.filter(
        (template) => template.id !== id,
      );

      // If the deleted template was selected, clear selection
      const newUI =
        data.ui.selectedTemplateId === id
          ? {
              selectedTemplateId:
    ...
    const deleteSnippet = (id: string) => {
      ...
      const updatedSnippets = data.snippets.filter(
        (snippet) => snippet.id !== id,
      );

      // If the deleted snippet was selected, clear selection
      const newUI =
        data.ui.selectedSnippetId === id
          ? {
              selectedSnippetId:
                updatedSnippets.length > 0 ? updatedSnippets[0].id : null,
            }
          : {};
    ...
    const deleteTag = (id: string) => {
      ...
      const updatedSelectedTags = data.ui.selectedTags.filter(
        (tagId) => tagId !== id,
      );
    ...
    const importData = (importedData: Partial<PromptManagerData>) => {
      ...
      const userTemplates = Array.from(
