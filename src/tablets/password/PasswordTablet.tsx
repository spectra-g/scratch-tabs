...
function generateSecurePassword(settings: PasswordSettings): string {
  ...
  const addCharset = (
    ...
    const chars = shouldExclude
      ? base.replace(new RegExp(`[${excludeChars}]`, "g"), "")
      : base;
    if (chars) {
      charset += chars;
  ...
  if (settings.useUppercase)
    addCharset(CHARSETS.upper, settings.excludeAmbiguous, "IO");
  if (settings.useLowercase)
    addCharset(CHARSETS.lower, settings.excludeAmbiguous, "il");
  if (settings.useNumbers)
    addCharset(CHARSETS.numbers, settings.excludeAmbiguous, "10");
...
function generateSecurePassphrase(settings: PassphraseSettings): string {
  ...
  return words
    .map((word) => {
      if (settings.capitalize === "title")
        return word.charAt(0).toUpperCase() + word.slice(1);
      if (settings.capitalize === "random" && Math.random() > 0.5)
...
const PasswordGeneratorUI: React.FC<{
  ...
  const passphraseSettings = data.passphraseSettings || {
    wordCount: 4,
    separator: "-",
    capitalize: "title" as Capitalization,
  };
  ...
  const updateSettings = (updates: Partial<PasswordSettings>) => {
    onChange({
      ...state,
      data: { ...data, passwordSettings: { ...passwordSettings, ...updates } },
    });
  };
  const updatePassphraseSettings = (updates: Partial<PassphraseSettings>) => {
    onChange({
      ...
      data: {
        ...data,
        passphraseSettings: { ...passphraseSettings, ...updates },
      },
    });
  };

  const handleCopyToHistory = () => {
    const passwordToSave = SensitiveDataManager.mask(
      SensitiveDataManager.unmask(currentPassword),
    );
  ...
  const deleteHistoryEntry = (id: string) => {
    onChange({
      ...state,
      data: { ...data, history: history.filter((entry) => entry.id !== id) },
    });
  };
      ...
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-lg p-4 space-y-4 flex-shrink-0">
        ...
        <div className="flex bg-gray-800/50 rounded-lg p-0.5 self-start">
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-sm transition-colors w-1/2 ${mode === "password" ? "bg-blue-500/20 text-blue-400" : "text-gray-400 hover:text-gray-300"}`}
          >
            Password
          </button>
          <button
            onClick={() =>
              onChange({ ...state, data: { ...data, mode: "passphrase" } })
            }
            className={`px-3 py-1.5 rounded-md text-sm transition-colors w-1/2 ${mode === "passphrase" ? "bg-blue-500/20 text-blue-400" : "text-gray-400 hover:text-gray-300"}`}
          >
            Passphrase
          </button>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <input
            ...
            onChange={(e) =>
              onChange({
                ...
                data: {
                  ...data,
                  currentPassword: SensitiveDataManager.mask(e.target.value),
                },
              })
            }
          ...
          <button
            ...
            className={`p-2 rounded-md transition-all duration-150 relative ${copiedId === "current" ? "text-green-400 hover:text-green-300" : "text-gray-400 hover:text-gray-200 hover:bg-gray-700/50"}`}
        ...
        <div className="flex items-center space-x-3 pt-1">
          <div className="flex-1 bg-gray-700/50 rounded-full h-1.5">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{
                width: `${Math.min(100, (passwordStrength.entropy / 128) * 100)}%`,
              }}
          ...
          <div className="text-xs text-gray-500">
            (Est. crack time: {passwordStrength.timeToCrack})
          </div>
        </div>
          ...
          {mode === "password" ? (
            <>
              <div className="flex items-center space-x-3">
                <label
                  htmlFor="length"
                  className="text-sm font-medium text-gray-300 w-28 flex-shrink-0"
                >
                  Length:
                </label>
                <input
                  ...
                  onChange={(e) =>
                    updateSettings({ length: parseInt(e.target.value) })
                  }
                  className="flex-1 h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer range-lg accent-blue-500"
                />
                <span className="text-sm font-mono bg-gray-700/60 px-2 py-0.5 rounded w-12 text-center flex-shrink-0">
                ...
                {[
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={
                        !!passwordSettings[key as keyof PasswordSettings]
                      }
                      onChange={(e) =>
                        updateSettings({ [key]: e.target.checked })
                      }
                      className="h-4 w-4 rounded border-gray-600 text-blue-500 focus:ring-blue-500/50 bg-gray-700 accent-blue-500"
                    />
            ...
            <>
              ...
              <div className="flex items-center space-x-3">
                ...
                <input
                  ...
                  maxLength={1}
                  className="bg-gray-700/60 rounded w-12 text-center p-1"
                />
              </div>
              <div className="flex items-center space-x-3">
                <label className="text-sm font-medium text-gray-300 w-28 flex-shrink-0">
                  Capitalization:
                </label>
                <select
                  value={passphraseSettings.capitalize}
                  onChange={(e) =>
                    updatePassphraseSettings({
                      capitalize: e.target.value as Capitalization,
                    })
                  }
                  className="bg-gray-700/60 rounded p-1 text-sm"
                >
      ...
      <div className="flex-grow flex flex-col min-h-0">
        <div className="flex justify-between items-center mb-3 flex-shrink-0 px-1">
          ...
          {history.length > 0 && (
            <button
              onClick={() =>
                onChange({ ...state, data: { ...data, history: [] } })
              }
              className="flex items-center space-x-1.5 px-3 py-1 bg-red-500/15 text-red-400 rounded-md hover:bg-red-500/25 transition-colors disabled:opacity-50 text-xs"
              title="Clear History"
            >
              <Trash2 size={14} />
              <span>Clear</span>
                ...
                {history.map((entry) => (
                  ...
                  >
                    <td className="px-4 py-2 font-mono">
                      <span
                        className="blur-[3px] select-none hover:blur-none transition duration-150"
                        title="Hover to reveal"
                      >
                        {SensitiveDataManager.unmask(entry.password)}
                      </span>
                    </td>
                    <td>
                      <input
  ...
  deserializeState(json: string): PasswordTabletState {
    ...
    try {
      ...
      if (parsed.type === "password" && parsed.data) {
        ...
        let migratedHistory = Array.isArray(loadedData.history)
          ? loadedData.history
          : [];
        migratedHistory = SensitiveDataManager.migrateObjectArray(
          migratedHistory,
          ["password"],
        );
        ...
        const finalData: PasswordGeneratorData = {
          ...
          passwordSettings: {
            ...defaultState.data.passwordSettings,
            ...loadedData.passwordSettings,
          },
          passphraseSettings: {
            ...defaultState.data.passphraseSettings,
            ...loadedData.passphraseSettings,
          },
          currentPassword: SensitiveDataManager.migrateField(
            loadedData.currentPassword || "",
          ),
        ...
        if (loadedData.complexity) {
          finalData.passwordSettings.useSpecial =
            loadedData.complexity === "complex";
          finalData.passwordSettings.length =
