import { RegexMatch, RegexGroup, RegexFlag, RegexError, RegexExplanation } from '../types';

export const DEFAULT_FLAGS: RegexFlag[] = [
  { flag: 'g', name: 'Global', description: 'Find all matches rather than stopping after the first match', enabled: true },
  { flag: 'i', name: 'Ignore Case', description: 'Case-insensitive matching', enabled: false },
  { flag: 'm', name: 'Multiline', description: '^/$ match line breaks', enabled: false },
  { flag: 's', name: 'Dot All', description: '. matches newline characters', enabled: false },
  { flag: 'u', name: 'Unicode', description: 'Unicode support', enabled: false },
  { flag: 'y', name: 'Sticky', description: 'Matches only from the index indicated by lastIndex', enabled: false },
];

export function createRegexFromPattern(pattern: string, flags: RegexFlag[]): RegExp | null {
  try {
...
export function validateRegex(pattern: string, flags: RegexFlag[]): RegexError | null {
  try {
    const flagString = flags.filter(f => f.enabled).map(f => f.flag).join('');
...
export function executeRegex(pattern: string, testString: string, flags: RegexFlag[]): RegexMatch[] {
      ...
      if (match.groups) {
        Object.entries(match.groups).forEach(([name, value]) => {
          if (value !== undefined) {
            ...
            const existingGroup = groups.find(g => g.start === start && g.match === value);
            if (existingGroup) {
...
export function explainRegex(pattern: string): RegexExplanation[] {
  ...
  const explainToken = (token: string, type: RegexExplanation['type'], description: string, start: number, end: number) => {
  ...
  while (pos < pattern.length) {
    ...
    switch (char) {
      ...
      case '$':
        ...
        break;
      case '.':
        explainToken(char, 'character-class', 'Any character except newline', start, pos + 1);
        break;
      case '*':
        explainToken(char, 'quantifier', 'Zero or more of the preceding element', start, pos + 1);
        break;
      case '+':
        explainToken(char, 'quantifier', 'One or more of the preceding element', start, pos + 1);
        break;
      case '?':
        explainToken(char, 'quantifier', 'Zero or one of the preceding element', start, pos + 1);
        break;
      case '\\':
        if (pos + 1 < pattern.length) {
          const nextChar = pattern[pos + 1];
          const escapeSequence = char + nextChar;
          explainToken(escapeSequence, 'escape', getEscapeDescription(nextChar), start, pos + 2);
      ...
      case '[':
        ...
        if (classEnd !== -1) {
          ...
          explainToken(charClass, 'character-class', `Character class: ${charClass}`, start, classEnd + 1);
          pos = classEnd;
        } else {
          explainToken(char, 'literal', `Literal "${char}"`, start, pos + 1);
        }
      ...
      case '{':
        ...
        if (quantEnd !== -1) {
          ...
          explainToken(quantifier, 'quantifier', `Repeat ${quantifier}`, start, quantEnd + 1);
          pos = quantEnd;
        } else {
          explainToken(char, 'literal', `Literal "${char}"`, start, pos + 1);
        }
