...
export function SnippetSelector({ selectedSnippet, onSnippetSelect }: SnippetSelectorProps) {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-300">
            {selectedSnippet ? 
              snippetsByCategory[Object.keys(snippetsByCategory).find(cat => 
                snippetsByCategory[cat].some(s => s.id === selectedSnippet)
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800/95 border border-gray-700/50 rounded-md shadow-lg backdrop-blur-sm z-50 max-h-80 overflow-y-auto custom-scrollbar">
          {categories.map(category => (
            <div key={category}>
              ...
              >
                <span className="text-gray-200 font-medium text-sm">{category}</span>
                <ChevronDown 
                  size={12} 
