import { convertBetweenDialects, normalizeExpression } from '../dialectConverter';
...
describe('dialectConverter', () => {
  describe('Day of Week Numbering', () => {
    ...
    test('converts Unix Monday (1) to Quartz Monday (2)', () => {
      ...
      const quartzExpression = convertBetweenDialects(unixExpression, 'unix', 'quartz');
    ...
    test('converts Quartz Monday (2) to Unix Monday (1)', () => {
      ...
      const unixExpression = convertBetweenDialects(quartzExpression, 'quartz', 'unix');
    ...
    test('converts all weekdays from Quartz to Unix', () => {
      ...
      testCases.forEach(({ quartz, unix, day }) => {
        ...
        const result = convertBetweenDialects(quartzExpression, 'quartz', 'unix');
  ...
  describe('Spring Dialect', () => {
    test('converts Unix to Spring with day-of-week conversion', () => {
      ...
      const springExpression = convertBetweenDialects(unixExpression, 'unix', 'spring');
  ...
  describe('AWS Dialect', () => {
    test('converts Unix to AWS (should keep same day-of-week numbering)', () => {
      ...
      const awsExpression = convertBetweenDialects(unixExpression, 'unix', 'aws');
  ...
  describe('User Reported Issues', () => {
    test('Quartz "0 0 0 ? * 2 *" should be Monday at midnight, not Tuesday', () => {
      ...
      const backToQuartz = convertBetweenDialects(unixExpression, 'unix', 'quartz');
...
describe('normalizeExpression', () => {
  ...
  test('normalizes Quartz expression', () => {
    ...
    expect(normalizeExpression('45 30 14 15 6 ? 2024', 'quartz')).toBe('45 30 14 15 6 ? 2024');
  });

  test('normalizes Spring expression', () => {
    expect(normalizeExpression('0 8 * * 1', 'spring')).toBe('0 0 8 * * 1');
    expect(normalizeExpression('45 30 14 15 6 ? extra', 'spring')).toBe('45 30 14 15 6 ?');
  });

  test('normalizes AWS expression', () => {
    expect(normalizeExpression('0 8 * * 1', 'aws')).toBe('0 8 * * 1 *');
    expect(normalizeExpression('30 14 15 6 * 2024 extra', 'aws')).toBe('30 14 15 6 * 2024');
  });
}); 