...
export const useCronEngine = (
  ...
  const createRawExpression = useCallback(
    (expr: CronExpression, currentDialect: CronDialect): string => {
      let rawParts: string[] = [];
  ...
  const savePattern = useCallback(
    (name: string, description?: string): CronPattern => {
  ...
  const copyAllTimes = useCallback(() => {
    ...
    const text = nextExecutions
      .map((execution) => execution.formatted)
      .join("\n");
