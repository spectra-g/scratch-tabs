...
export const SegmentedBuilder: React.FC<SegmentedBuilderProps> = ({
  ...
  const getFieldError = (fieldName: string) => {
    const fieldKey = fieldName.toLowerCase().replace(/\s+/g, '') as keyof CronExpression;
    return validationErrors.find(error => error.field === fieldKey);
  };
  
  return (
    <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
      <h3 className="text-sm font-medium text-gray-300 mb-4">Segmented Builder</h3>
          ...
          return (
            <div key={index} className="space-y-1">
              ...
              <div className="flex space-x-2">
                <select
                  value={fieldOptions.find(opt => opt.value === fieldValue)?.value || 'custom'}
...
function getYearOptions() {
  ...
  return [
    ...
    { value: `${currentYear}-${currentYear + 5}`, label: `${currentYear} to ${currentYear + 5}` },
  ];
}