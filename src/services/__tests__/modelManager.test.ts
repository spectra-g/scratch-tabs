...
describe('ModelManager', () => {
  ...
  describe('content synchronization', () => {
    ...
    it('should sync content changes to store', async () => {
      ...
      (require('../../stores/tabsStore').useTabsStore.getState as jest.Mock).mockReturnValue({
        updateTabContent,
      });

      let capturedListener: (() => void) | undefined;
      (mockModel.onDidChangeContent as any).mockImplementation((listener: () => void) => {
  ...
  describe('language updates', () => {
    it('should update model language', async () => {
      ...
      expect(mockMonaco.editor.setModelLanguage).toHaveBeenCalledWith(mockModel, 'typescript');
    });
    ...
    it('should handle language update for non-existent model', () => {
      ...
      expect(() => modelManager.updateModelLanguage('non-existent', 'typescript')).not.toThrow();
    });
    ...
    it('should handle language update for disposed model', async () => {
      ...
      expect(() => modelManager.updateModelLanguage('test-tab', 'typescript')).not.toThrow();
    });
  });
  ...
  describe('content updates', () => {
    ...
    it('should handle content update for non-existent model', () => {
      // Non-existent models don't throw errors, they just don't do anything
      expect(() => modelManager.updateModelContent('non-existent', 'new content')).not.toThrow();
    });
  });
  ...
  describe('error handling', () => {
    it('should handle model creation errors gracefully', async () => {
      ...
      await expect(modelManager.get(testTab)).rejects.toThrow('Model creation failed');
    });
  });
}); 