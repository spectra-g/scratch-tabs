    ...
    "categories": [
      {
        ...
        "changes": [
          ...
          "**Automatic State Restoration:** All sidebar preferences load instantly on app startup from IndexedDB with zero user action required",
      ...
      },
      {
        "name": "Improvements",
        "changes": [
          "**Performance-Optimized Saves:** Sidebar width changes during resize are debounced (1 second) to prevent database thrashing while maintaining immediate visual feedback",
          "**Graceful Error Handling:** Corrupted or missing persistence data falls back to sensible defaults without breaking the UI",
          "**Type-Safe Persistence:** Strongly-typed persistence layer ensures data integrity with proper SetArray serialization for workspace IDs",
          "**CSV Parsing Enhancement:** CSV column extraction now properly handles quoted fields containing delimiters, escaped quotes, and mixed quoted/unquoted data - fully RFC 4180 compliant parsing with state machine logic",
          "**Universal Crypto Implementation:** HMAC generation uses Web Crypto API (works in modern browsers and Node.js 15+) with automatic fallback to Node.js crypto for legacy test environments",
          "**Modular Pipeline Architecture:** New operations organized by domain (web.ts, code.ts, dataFormats.ts, crypto.ts) for better maintainability and discovery - each module self-registers with comprehensive test coverage (950+ tests across all operations)"
        ]
      },
      {
        "name": "Pipeline Operations Catalog",
        "changes": [
          "**Web Operations:** HTML strip tags with line break preservation, JWT decode with timestamp conversion and Bearer prefix support",
          "**Code Operations:** SQL escape for safe string insertion, SQL prettify with keyword uppercasing and indentation control, CSS prettify with configurable spacing and comma formatting",
          "**Data Formats:** YAML to JSON conversion with indent control, JSON to YAML with double-quote preference, CSV column extraction by index or header name with multi-delimiter support (comma/tab/semicolon/pipe)",
          "**Text Operations:** Slugify for URL-friendly strings with separator options, remove diacritics for ASCII normalization, frequency analysis for lines/words/characters with CSV/JSON export",
          "**Extraction:** Regex capture group extraction with configurable group index, unique filtering, and full flag support (g/i/m)",
          "**Encoding:** Quoted-printable decoder for MIME email content with soft line break handling and hex character decoding",
          "**Cryptography:** HMAC generation with SHA-1/256/384/512 support and hex/base64 output, base converter for binary/octal/decimal/hexadecimal with prefix options"
        ]
      }
    ]
  },
