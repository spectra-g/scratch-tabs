...
export const MaskedCell: React.FC<MaskedCellProps> = React.memo(({
  ...
  const handleToggleMaskClick = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    onToggleMask();
  }, [onToggleMask]);
  ...
  return (
    ...
    >
      <span className={`text-sm truncate flex-1 mr-2 select-none transition duration-150 ${shouldShowMasked ? 'blur-[3px] hover:blur-none' : 'text-gray-200'}`}
