...
describe("CacheStore", () => {
  ...
  describe("Cache Split View", () => {
    ...
    it("should overwrite existing cached split view", () => {
      ...
      useCacheStore
        .getState()
        .cacheSplitViewForWorkspace(workspaceId, firstSplitView);
      expect(useCacheStore.getState().cachedSplitView?.splitView.id).toBe(
        "split-view-1",
      );
      ...
      useCacheStore
        .getState()
        .cacheSplitViewForWorkspace(workspaceId, secondSplitView);
      expect(useCacheStore.getState().cachedSplitView?.splitView.id).toBe(
        "split-view-2",
      );
    });
    ...
    it("should handle different workspace IDs", () => {
      ...
      useCacheStore
        .getState()
        .cacheSplitViewForWorkspace(firstWorkspaceId, splitView);
      expect(useCacheStore.getState().cachedSplitView?.workspaceId).toBe(
        firstWorkspaceId,
      );
      ...
      useCacheStore
        .getState()
        .cacheSplitViewForWorkspace(secondWorkspaceId, splitView);
      expect(useCacheStore.getState().cachedSplitView?.workspaceId).toBe(
        secondWorkspaceId,
      );
    });
  ...
  describe("Function References", () => {
    it("should provide consistent function references", () => {
      ...
      expect(store1.cacheSplitViewForWorkspace).toBe(
        store2.cacheSplitViewForWorkspace,
      );
  ...
  describe("State Immutability", () => {
    it("should not mutate the original splitView object", () => {
      ...
      useCacheStore
        .getState()
        .cacheSplitViewForWorkspace(workspaceId, originalSplitView);

      // Original should not be modified
      expect(originalSplitView).toEqual(originalCopy);

      // Cached version should be a copy
      const cachedSplitView =
        useCacheStore.getState().cachedSplitView?.splitView;
