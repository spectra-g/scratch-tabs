...
const mockWorkspaceStore = useWorkspaceStore as jest.Mocked<typeof useWorkspaceStore>;
...
describe('TabsStore', () => {
  ...
  describe('Update Tab Content', () => {
    ...
    it('should not update if tab is not found', () => {
      ...
      useTabsStore.getState().updateTabContent('non-existent-id', 'New content');
  ...
  describe('Update Tab Language', () => {
    ...
    it('should not update if tab is not found', () => {
      ...
      useTabsStore.getState().updateTabLanguage('non-existent-id', 'python', true);
  ...
  describe('Duplicate Tab', () => {
    ...
    it('should return empty string if original tab is not found', () => {
      const duplicateId = useTabsStore.getState().duplicateTab('non-existent-id');
  ...
  describe('Workspace Filtering', () => {
    it('should filter tabs by workspace ID', () => {
      const workspaceTab1 = { ...mockTab, id: 'workspace-tab-1', workspaceId: 'target-workspace' };
      const workspaceTab2 = { ...mockTab, id: 'workspace-tab-2', workspaceId: 'target-workspace' };
      const otherTab = { ...mockTab, id: 'other-tab', workspaceId: 'other-workspace' };
      ...
      const state = useTabsStore.getState();
      const targetWorkspaceTabs = state.tabs.filter(tab => tab.workspaceId === 'target-workspace');
      
      expect(targetWorkspaceTabs).toHaveLength(2);
      expect(targetWorkspaceTabs.map(t => t.id)).toEqual(['workspace-tab-1', 'workspace-tab-2']);
    });
  });
  ...
  describe('Integration with incrementSetting', () => {
    ...
    it('should handle incrementSetting errors gracefully', async () => {
      ...
      (incrementSetting as jest.Mock).mockRejectedValue(new Error('Increment error'));
      ...
      expect(incrementSetting).toHaveBeenCalledWith('tabs.created.total');
      expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to increment tab counter:', expect.any(Error));
