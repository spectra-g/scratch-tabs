...
export const useSearchStore = create<SearchState>()(
  persist(
    (set, get) => ({
      ...
      toggleSearch: (initialQuery = "") =>
        set((state) => {
          const newState = !state.isOpen;
          return {
            isOpen: newState,
            query:
              newState && initialQuery
                ? initialQuery
                : newState
                  ? state.query
                  : "", // Populate if opening with initial query
            error: null, // Clear error on open/close
            statusMessage: newState
              ? initialQuery
                ? "Searching..."
                : "Enter text to search."
              : "Search closed.", // Update status
            // Don't clear results on open, maybe user wants to refine
          };
        }),
      setQuery: (query) =>
        set({ query, error: null, selectedResultIndex: null }), // Reset selection on query change
      setScope: (scope) =>
        set({ scope, error: null, selectedResultIndex: null }),
      ...
      setTitleFilter: (filter) =>
        set({ titleFilter: filter, error: null, selectedResultIndex: null }),
      ...
      setResults: (results) =>
        set({ results, selectedResultIndex: results.length > 0 ? 0 : null }), // Auto-select first result
      ...
      closeSearch: () =>
        set({ isOpen: false, statusMessage: "Search closed." }), // Explicit close action
    }),
    {
