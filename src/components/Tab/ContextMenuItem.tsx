...
export const ContextMenuItem: React.FC<ContextMenuItemProps> = ({ item }) => {
    ...
    const [submenuPosition, setSubmenuPosition] = useState({ top: '-5px', left: '100%', bottom: 'auto' }); // Added bottom
    ...
    useEffect(() => {
        if (isSubmenuVisible && itemRef.current && submenuRef.current) {
            ...
            if (itemRect.top + submenuHeight > windowHeight - 20) { // 20px buffer
    ...
    return (
        ...
        >
            ...
            {item.submenu && isSubmenuVisible && (
                <div
                    ref={submenuRef}
                    className="absolute left-full ml-1 bg-gray-700 border border-gray-600 rounded shadow-lg z-[60] py-1 min-w-[150px] max-h-[300px] overflow-y-auto custom-scrollbar"
