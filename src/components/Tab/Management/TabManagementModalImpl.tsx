    ...
    switch (sortOption) {
      case SORT_OPTIONS.CURRENT:
        ...
        const positionMap = new Map(
          currentOrder.map((id, index) => [id, index]),
        );
...
export const useDuplicateTabs = (
  ...
  return useMemo(() => {
    ...
    const currentViewTabs = activeWorkspaceTabs.filter(
...
export const handleDragEnd = (
  ...
  if (overDataType === "workspace" && overData?.workspaceId) {
    ...
    if (targetWorkspaceId && targetWorkspaceId !== activeWorkspaceId) {
      ...
      const itemsToMove = filteredTabs.filter((tab) =>
        draggedTabIdsToMove.includes(tab.id),
      );
      const otherItemsInCurrentWorkspace = filteredTabs.filter(
        (tab) => !draggedTabIdsToMove.includes(tab.id),
      );
      const finalNewOrderInCurrentWorkspace = [
        ...otherItemsInCurrentWorkspace,
        ...itemsToMove,
      ];
      handleApplyCurrentOrder(finalNewOrderInCurrentWorkspace);
    }
  } else if (overDataType === "tab" || overDataType === "group") {
    const targetItemWorkspaceId =
      overDataType === "tab"
        ? overData?.tab?.workspaceId
        : overData?.groupWorkspaceId;
    if (targetItemWorkspaceId === activeWorkspaceId) {
      // Reordering within the current workspace
      const itemsToMove = filteredTabs.filter((tab) =>
        draggedTabIdsToMove.includes(tab.id),
      );
      const currentWorkspaceFilteredTabs = filteredTabs.filter(
        (t) => t.workspaceId === activeWorkspaceId,
      );
      const otherItemsInCurrentWorkspace = currentWorkspaceFilteredTabs.filter(
        (tab) => !draggedTabIdsToMove.includes(tab.id),
      );

      let finalNewOrderInCurrentWorkspace: Tab[];
      const targetIndexInOthers = otherItemsInCurrentWorkspace.findIndex(
        (tab) => tab.id === overId,
      );
      ...
      if (targetIndexInOthers !== -1) {
        ...
        // Dropped on a group area or end of list
        finalNewOrderInCurrentWorkspace = [
          ...otherItemsInCurrentWorkspace,
          ...itemsToMove,
        ];
