...
export const useTabManagementEngine = (
  isOpen: boolean,
  onClose: () => void,
): TabManagementEngine => {
  ...
  const [editingTabIdForModal, setEditingTabIdForModal] = useState<
    string | null
  >(null);
  ...
  const [activeDragItemData, setActiveDragItemData] = useState<Tab | null>(
    null,
  );
  const { cacheSplitViewForWorkspace } = useCacheStore();
  const {
    isLocked: isTabManagementActionInProgress,
    withLock: withActionLock,
  } = useActionLock();
  ...
  const workspacesWithCounts = useMemo(() => {
    ...
    return workspaces.map((ws) => ({
      ...ws,
      tabCount: allApplicationTabs.filter((tab) => tab.workspaceId === ws.id)
        .length,
  ...
  const handleModifiedApplyCurrentOrder = (
    eventOrTabs: React.MouseEvent<HTMLButtonElement> | Tab[],
  ) => {
    setSortOption(
      applyCurrentOrderHelper(eventOrTabs, filteredTabs) as SortOption,
    );
  };
  ...
  const handleTogglePinSelectedTabs = () => {
    ...
    const firstTab = localTabs.find(
      (t) => t.id === Array.from(selectedTabIds)[0],
    );
  ...
  const handleDeleteWorkspace = (workspaceId: string) => {
    ...
    setConfirmationDialog({
      ...
      message: `Are you sure you want to delete "${workspace.name}"? All its tabs will be deleted.`,
  ...
  const handleRemoveDuplicates = () => {
    ...
    setConfirmationDialog({
      ...
      message: `Found ${groups.length} groups of duplicates. Keep the newest from each group?`,
      confirmText: "Remove Duplicates",
      cancelText: "Cancel",
      onConfirm: () => {
        groups.forEach((group) => {
          const sorted = [...(group as Tab[])].sort(
            (a, b) => b.lastModified - a.lastModified,
          );
          for (let i = 1; i < sorted.length; i++)
            useRootStore.getState().removeTab(sorted[i].id);
        });
        setConfirmationDialog((prev) => ({ ...prev, isOpen: false }));
      },
    });
  ...
  const handleSwitchWorkspaceAndKeepModal = (
    workspaceId: string,
    event: React.MouseEvent,
  ) => {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    setDraggedTabIds(
      selectedTabIds.has(tabId) ? new Set(selectedTabIds) : new Set([tabId]),
    );
  };
