...
export const useSearchEngine = (): SearchEngine => {
  ...
  const { switchWorkspace, activeWorkspaceId: currentActiveWsId } =
    useWorkspaceStore();
  ...
  const currentSelectedResultIndex = useSearchStore(
    (state) => state.selectedResultIndex,
  );
  const selectedResult =
    currentSelectedResultIndex !== null &&
  ...
  const matchesTitleFilter = useCallback(
    (tabTitle: string) => {
      ...
      setTimeout(async () => {
        try {
          ...
          if (currentScope === "activeWorkspace") {
            tabsToSearch = allTabsInCurrentWorkspace.filter(
          ...
          if (foundResults.length === 0) {
            ...
            setStatusMessage(
              `${foundResults.length} match${foundResults.length === 1 ? "" : "es"} in ${uniqueTabs.size} file${uniqueTabs.size === 1 ? "" : "s"}`,
            );
  ...
  const handleResultNavigation = useCallback(
    async (result: SearchResult) => {
      ...
      const targetTabSide = splitViewState.leftTabs.includes(result.tabId)
        ? "left"
        : splitViewState.rightTabs.includes(result.tabId)
          ? "right"
          : null;
      ...
      if (targetTabSide === "left") {
        ...
        if (allCurrentTabs.find((t: Tab) => t.id === result.tabId)) {
          ...
          if (!splitViewState.leftTabs.includes(result.tabId)) {
            console.warn(
              `Tab ${result.tabId} activated but was not in expected splitView side.`,
            );
          }
        } else {
  ...
  const handleLangCheckboxChange = useCallback(
