...
export const EditorInstance: React.FC<EditorInstanceProps> = ({ side, activeTab, onEditorReady }) => {
  ...
  const handleEditorChange = (value: string | undefined) => {
    ...
    // For large content, defer the state update to prevent UI blocking
    if (newContent.length > LARGE_CONTENT_THRESHOLD) {
      // Defer the update to prevent React reconciliation from blocking
      setTimeout(() => {
        updateTabContent(currentTabId, newContent);
      }, 0);
    } else {
      // Small content can be updated immediately
      updateTabContent(currentTabId, newContent);
    }

    // Language detection and tablet selector logic
