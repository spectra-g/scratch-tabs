...
export const ExportWorkspacesModal: React.FC<ExportWorkspacesModalProps> = ({ isOpen, onClose }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-850 p-6 rounded-lg shadow-2xl w-full max-w-lg border border-gray-700/50 max-h-[80vh] flex flex-col">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-white">Export Workspaces</h2>
        ...
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-sm text-gray-400">{selectedIds.size} of {workspaces.length} selected</span>
        </div>

        {isLoading && workspaces.length > 0 && Object.keys(tabCounts).length === 0 && <p className="text-center text-gray-300 py-4">Loading tab counts...</p>}
        ...
        <div className="overflow-y-auto flex-grow custom-scrollbar pr-1 space-y-2 mb-4">
          {workspaces.map(ws => (
            ...
            >
              {selectedIds.has(ws.id) ? <CheckSquare size={20} className="mr-3 text-blue-400 flex-shrink-0" /> : <Square size={20} className="mr-3 text-gray-500 flex-shrink-0" />}
              <span className="text-gray-100 truncate flex-grow">{ws.name}</span>
              <span className="text-xs text-gray-400 ml-2 flex-shrink-0">
                ({tabCounts[ws.id] !== undefined ? `${tabCounts[ws.id]} tab${tabCounts[ws.id] === 1 ? '' : 's'}` : '...'})
              </span>
            </div>
          ))}
          {workspaces.length === 0 && <p className="text-center text-gray-400 py-4">No workspaces found.</p>}
        </div>
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-700/50">
          ...
          >
            {isLoading ? 'Exporting...' : `Export Selected (${selectedIds.size})`}
          </button>
        </div>
      </div>
    </div>
