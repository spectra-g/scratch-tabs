...
export const ImportWorkspacesModal: React.FC<ImportWorkspacesModalProps> = ({
  isOpen,
  onClose,
}) => {
    ...
    async (acceptedFiles: File[]) => {
      if (acceptedFiles.length > 0) {
        ...
        if (!file.name.endsWith(".scratch")) {
          setProcessingError(
            "Invalid file type. Please select a '.scratch' file.",
          );
      ...
      <div className="bg-themed p-6 rounded-lg shadow-2xl w-full max-w-lg border border-themed max-h-[80vh] flex flex-col">
        ...
        {step === "selectFile" && (
          <div
            ...
            ${isDragActive ? "border-blue-500 bg-blue-500/10" : "border-themed-light hover:border-themed"}`}
          >
            <input {...getInputProps()} />
            <div className="flex flex-col items-center justify-center text-center">
              <UploadCloud size={48} className="text-themed-muted mb-3" />
              {isDragActive ? (
                <p className="text-blue-500">Drop the .scratch file here...</p>
              ) : (
        ...
        {step === "loading" && (
          <div className="flex flex-col items-center justify-center py-10">
            ...
            <p className="text-themed-secondary">Processing import file...</p>
          </div>
        )}

        {step === "summary" && summary && (
          <div className="flex-grow overflow-y-auto custom-scrollbar pr-1">
            <h3 className="text-lg font-medium text-themed mb-3">
              Import Summary
            </h3>
            ...
            {summary.importedWorkspaces.length > 0 && (
              <div className="space-y-2">
                <h4 className="font-semibold text-themed-secondary mb-1">
                  Successfully Processed Workspaces:
                </h4>
                {summary.importedWorkspaces.map((item, idx) => (
                  <div key={idx} className="p-3 bg-themed-secondary rounded-md">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        />
                        <span className="text-themed">{item.name}</span>
                      </div>
        ...
        <div className="flex justify-end mt-6 pt-4 border-t border-themed">
          <button
            onClick={handleClose}
            className="px-4 py-2 text-sm bg-themed-secondary hover:bg-themed-hover text-themed rounded-md transition-colors"
          >
