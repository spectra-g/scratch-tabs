...
const getModelValueInRange = (model: monaco.editor.ITextModel | null, range: monaco.Range): string => {
...
export const useMacroEngine = (
  ...
  useEffect(() => {
    ...
    try {
        ...
        try {
          ...
          if (shift) {
            ...
            if (key.toLowerCase() === 'v') {
              ...
              const value = selection && !selection.isEmpty() ? getModelValueInRange(model, selection) : "";
          ...
          console.warn('[MacroEngine] Failed to handle key down event:', error);
        }
      }));
      ...
      console.error('[MacroEngine] Failed to setup recording listeners:', error);
    }
    ...
    try {
      ...
      try {
        ...
        for (const action of recordedActions) {
          ...
          const needsSelection = action.type === ACTION_TYPE.CHAR || 
  ...
  const handlePlayToEnd = useCallback(async () => {
    ...
    try {
      ...
      if (!model || isModelDisposed(model) || !endPosOfPreviousIteration) { // Ensure we have model and start position
