...
export const useLanguageDetection = (
  ...
  onLanguageDetectedOnSignificantChange?: (
    tabId: string,
    language: string,
  ) => void,
) => {
  ...
  const performDetectionAndUpdate = useCallback(
    (
      ...
      const isSignificantChange =
        ...
        (trimmedNewContent.length > 0 &&
          trimmedOldContent.length > 0 && // Avoid triggering on first char
    ...
    [updateTabLanguage, onLanguageDetectedOnSignificantChange],
