...
fun main(args: Array<String>) = runBlocking { // Entry point
  ...
  detect(content: string): DetectionResult {
    ...
    if (/^\s*#![^\r\n]*(scala|python|ruby|bash|sh)/i.test(trimmedContent)) {
      // Shebangs for other common scripts
      return { match: false, confidence: 0.0, matchedDefinitive: false };
    }
    ...
    if (
      /^(?:[A-Za-z_][\w.$]*(?:Error|Exception|Panic|Traceback))/m.test(
        trimmedContent,
      )
    ) {
    ...
    if (/\b(case\s+class|case\s+object|trait)\b/g.test(content)) {
      // Strong Scala keywords
      confidenceScore -= 0.7; // Heavy penalty
    }
    if (
      /\bdef\s+[a-zA-Z_][\w!?=~<>*+\-%&^|]*\s*(?:\[[^\]]+\])?\s*(?:\([^)]*\))*\s*:\s*[\w\[\],.<>:]+\s*=/g.test(
        content,
      ) &&
      !/\bfun\b/.test(content)
    ) {
    ...
    if (/\s+with\s+[\w.:<>\[\]]+\s*\{/g.test(content)) {
      // Scala `with Trait {`
      confidenceScore -= 0.5;
    }
    if (/\bimport\s+[\w.]+\._\b/g.test(content)) {
      // Scala wildcard import `import foo._`
      confidenceScore -= 0.3;
    }
    if (/\bs(?:""|"|')/g.test(content) && !content.includes("$")) {
      // Scala s"" string interpolator without Kotlin's $
      confidenceScore -= 0.4;
    }
    // --- End Anti-Patterns ---
    ...
    const definitivePatterns = [
      {
        ...
        perMatch: 0.02,
        maxMatches: 1,
      }, // Less unique than Java's
      {
        pattern: /^\s*import\s+[\w.*]+(?:\s+as\s+\w+)?/m,
        weight: 0.1,
        perMatch: 0.01,
        maxMatches: 5,
      }, // Also less unique
      {
        pattern: /\bfun\s+[a-zA-Z_][\w<>.:,\s=?]*\s*\(/g,
        weight: 0.35,
        perMatch: 0.05,
        specific: true,
        maxMatches: 5,
      ...
      {
        pattern:
          /\b(val|var)\s+[a-zA-Z_][\w<>.:,\s=?]*\s*(?::\s*[\w<>?.[\]]+)?\s*(?:=\s*[^;{\n]+|by\s+\w+)/g,
        weight: 0.3,
        perMatch: 0.05,
        specific: true,
        maxMatches: 10,
      }, // val/var name: Type = or val/var name by delegate
      {
        pattern: /\bdata\s+class\s+\w+/g,
        weight: 0.3,
        perMatch: 0.1,
        specific: true,
        maxMatches: 2,
      ...
      {
        pattern: /\bobject\s+\w+(?:\s*:\s*[\w<>]+)?\s*(?:,\s*[\w<>]+)*\s*\{/g,
        weight: 0.25,
        perMatch: 0.05,
        specific: true,
        maxMatches: 2,
      }, // object Foo or object Bar : Baz
      {
        pattern:
          /\b(interface|enum\s+class|sealed\s+class|annotation\s+class)\s+\w+/g,
        weight: 0.2,
        perMatch: 0.05,
        specific: true,
        maxMatches: 3,
      ...
      {
        pattern: /\b(lateinit\s+var|lazy\s*(?:\{\s*\}|\bval\b))/g,
        weight: 0.3,
        perMatch: 0.1,
        specific: true,
        maxMatches: 2,
      ...
      {
        pattern:
          /\b(?:suspend\s+fun|coroutineScope|launch|async|delay|runBlocking)\b/g,
        weight: 0.25,
        perMatch: 0.05,
        specific: true,
        maxMatches: 3,
      ...
      {
        pattern:
          /@(?:JvmStatic|JvmOverloads|JvmField|JvmName|file:|param:|field:|get:|set:)\b/g,
        weight: 0.15,
        perMatch: 0.03,
        specific: true,
        maxMatches: 3,
    ...
    const otherAntiPatterns = [
      ...
      }, // Python def, ensure not confused with Kotlin fun
    ];
    ...
    const isMatch =
      (strongSignalFound &&
        specificKotlinHits >= 1 &&
        confidenceScore >= 0.4) ||
      (specificKotlinHits >= 2 && confidenceScore >= 0.5) ||
      (patternsMatched >= 3 &&
        confidenceScore >= 0.6 &&
        !content.includes("def ")); // Higher bar if no strong signals
...
const kotlinDetector = new KotlinFormatDetector();
formatRegistry.register(kotlinDetector);
