 ...
 */
export class JavaFormatDetector extends BaseFormatDetector {
  id = "java";
  name = "Java";
  extensions = ["java"];
  ...
  detect(content: string): DetectionResult {
    ...
    if (definitiveMatch && confidenceScore > 0.4) {
      return {
        match: true,
        confidence: Math.min(1.0, 0.6 + confidenceScore * 0.5),
      };
    }
    ...
    const commonPatterns = [
      {
        pattern:
          /\b(public|private|protected)\s+(static\s+)?(final\s+)?\w+\s+\w+\s*\(.*\)\s*\{/i,
        weight: 0.15,
      }, // Method signature
      {
        pattern:
          /\b(int|boolean|String|void|double|float|long|char|byte)\s+\w+(\s*\[\])?\s*;/i,
        weight: 0.15,
      }, // Variable declaration with types
      { pattern: /\bnew\s+\w+\s*\([^)]*\)/i, weight: 0.1 },
      {
        pattern: /\btry\s*\{[\s\S]*\}\s*catch\s*\((\w+Exception|\w+)\s+\w+\)/i,
        weight: 0.15,
      }, // Try-catch
      {
        pattern: /\bfor\s*\(\s*(int|String)\s+\w+\s*:\s*\w+\s*\)/i,
        weight: 0.1,
      }, // Enhanced for loop
...
const javaDetector = new JavaFormatDetector();
formatRegistry.register(javaDetector);
