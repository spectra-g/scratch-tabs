...
describe("Registry Format Detection", () => {
  ...
  describe("Cross-Format Detection Issues", () => {
    test("R and CSV should NOT detect JSON with dot notation properties", () => {
      ...
      // Get all detection results
      const results = getAllDetectionResults(jsonContent);
      
      // Find R and CSV results specifically
      const rResult = results.find(r => r.id === 'r');
      const csvResult = results.find(r => r.id === 'csv');
      
      // The main fix: R should NOT match this content anymore
      expect(rResult).toBeDefined();
      expect(rResult!.match).toBe(false);
      expect(rResult!.confidence).toBe(0);
      
      // CSV should NOT match this content (single line)
