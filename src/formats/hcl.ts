...
data "aws_ami" "latest_amazon_linux" {
  ...
  detect(content: string): DetectionResult {
    ...
    const attributePattern =
      /^\s*([a-zA-Z_][\w-]*)\s*=\s*(?:".*?"|'.*?'|\d+\.?\d*|true|false|\[.*?\]|\{.*?\}|<<-?\w+[\s\S]*?\n\w+|[\w.-]+)/gm;
    ...
    const antiPatterns = [
      ...
      {
        pattern: /\b(function|class|public|private|protected|static)\s+\w+/i,
        weight: -0.4,
      }, // Common OOP keywords from other langs
      {
        pattern: /^\s*import\s+(?:[\w.{}\s,]+from\s*)?['"][\w./-]+["'];?/im,
        weight: -0.3,
      }, // JS/TS/Python/Java import
    ...
    const isMatch =
      ...
      (patternsMatched >= 2 && confidenceScore >= 0.6);
...
const hclDetector = new HclFormatDetector();
formatRegistry.register(hclDetector);
