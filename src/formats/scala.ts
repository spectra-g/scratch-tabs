...
`;
  ...
  detect(content: string): DetectionResult {
    ...
    if (/\bif\s+__name__\s*==\s*(['"])__main__\1\s*:/m.test(content)) {
      // Python main guard
      antiPatternPenaltyScore -= 0.8;
    }
    if (
      /^\s*"""[\s\S]*?"""|^\s*'''[\s\S]*?'''/m.test(content) &&
      !content.match(/s"""|f"""|raw"""/)
    ) {
      // Python docstrings/multiline strings, but not Scala's raw/interpolated
      antiPatternPenaltyScore -= 0.5;
    }
    if (/\b(elif|else\s*:)\b/g.test(content) && !content.match(/\belse\s*\{/)) {
      // Python elif/else:, Scala uses `else if` or `else {`
      antiPatternPenaltyScore -= 0.4;
    }
    if (
      /\b(True|False|None)(?!\w)/g.test(content) &&
      !content.match(/\b(Some|None)\(/)
    ) {
      // Python bool/None, ensure not Scala's Option.None
      antiPatternPenaltyScore -= 0.3;
    }
    if (
      /\[.+?\s+for\s+\w+\s+in\s+.+?\]/g.test(content) &&
      !/\bfor\s*\{.*\}\s*yield\b/.test(content)
    ) {
      // Python list comprehension vs Scala for/yield
      antiPatternPenaltyScore -= 0.5;
    }
    if (
      /^\s*from\s+[\w.]+\s+import\s+(?:\*|\w+|\([\w\s,]+\))$/gm.test(content) &&
      !/^\s*import\s+[\w.{}]/.test(content)
    ) {
      // Python from...import, if not also Scala-style import
      antiPatternPenaltyScore -= 0.4;
    }
    if (
      /\s*->\s*([\w\[\],.:\s|]+?)(?:\s*#.*)?$/gm.test(content) &&
      !/\s*:\s*[\w\[\],.<>:"']+\s*=/.test(content)
    ) {
      // Python return type hint `-> Type:` vs Scala `): Type =`
      antiPatternPenaltyScore -= 0.3;
    }
    ...
    const definitivePatterns = [
      ...
      {
        pattern:
          /\b(class|trait|object)\s+[A-Z_][\w]*(?:\[[^\]]+\])?(?:\s*\([^)]*\))?(?:\s+extends\s+[\w.:<>\[\]]+)?(?:\s+with\s+[\w.:<>\[\]]+)*\s*\{?/g,
        weight: 0.4,
        perMatch: 0.1,
        specific: true,
        maxMatches: 4,
      ...
      {
        pattern:
          /\b(?:val|var)\s+[a-zA-Z_][\w]*\s*:\s*[\w\[\],.<>:"'\s]+\s*(?:=\s*[^;{\n]*)?/g,
        weight: 0.35,
        perMatch: 0.05,
        specific: true,
        maxMatches: 10,
      ...
      {
        pattern:
          /\bdef\s+[a-zA-Z_][\w!?=~<>*+\-%&^|]*\s*(?:\[[^\]]+\])?\s*(?:\([^)]*\))*\s*:\s*[\w\[\],.<>:"'\s]+\s*=/g,
        weight: 0.4,
        perMatch: 0.05,
        specific: true,
        maxMatches: 5,
      ...
      {
        pattern: /\bcase\s+(?:class|object)\s+\w+/g,
        weight: 0.4,
        perMatch: 0.1,
        specific: true,
        maxMatches: 3,
      ...
      {
        pattern:
          /\b(private\[\w+\]|protected\[\w+\]|override|final|sealed|implicit|lazy|abstract\s+override)\b/g,
        weight: 0.2,
        perMatch: 0.02,
        maxMatches: 5,
        specific: true,
      ...
      {
        pattern: /\b(for|yield)\s*(?:\{|\()[\s\S]*?(?:\}|\))\s*(?:yield)?/g,
        weight: 0.3,
        perMatch: 0.03,
        specific: true,
        maxMatches: 3,
    ...
    for (const dp of definitivePatterns) {
      ...
      if (matches) {
        ...
        if (dp.perMatch) {
          confidenceScore +=
            Math.min(matches.length, dp.maxMatches || 5) * dp.perMatch;
        }
        patternsMatched++;
        if (dp.specific) {
          strongSignalFound = true;
    ...
    const collectionPatterns = [
      {
        pattern:
          /\.\s*(map|flatMap|filter|foreach|reduce|foldLeft|collect|groupBy|zip|headOption|find|exists|forall|mkString|to[A-Z]\w*)\b/g,
        weight: 0.15,
        perMatch: 0.01,
        maxMatches: 10,
      },
      {
        pattern:
          /\b(List|Seq|Vector|Map|Set|Option|Some|None|Try|Success|Failure|Future|Either|Left|Right)\s*\(?/g,
        weight: 0.2,
        perMatch: 0.02,
        maxMatches: 5,
      },
    ...
    for (const cp of collectionPatterns) {
      ...
      if (matches) {
        ...
        if (cp.perMatch) {
          confidenceScore +=
            Math.min(matches.length, cp.maxMatches || 5) * cp.perMatch;
        }
        patternsMatched++;
      }
    }
    ...
    if (
      ...
      (content.includes("val ") || content.includes("var ")) &&
      content.includes("=>") &&
      content.includes(": ") &&
      content.includes(" = ")
    ) {
    ...
    const isMatch =
      (strongSignalFound && specificScalaHits >= 1 && confidenceScore >= 0.5) ||
      (specificScalaHits >= 2 && confidenceScore >= 0.6) ||
      (patternsMatched >= 4 && confidenceScore >= 0.7);
...
const scalaDetector = new ScalaFormatDetector();
formatRegistry.register(scalaDetector);
